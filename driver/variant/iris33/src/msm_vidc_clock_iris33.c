...
{
	u32 (*frequency_table_pointer)[6];
	u32 (*bitrate_table_2stage_pointer)[10];
	/*
	 * VSP calculation
	...
	if (codec_input.pipe_num == 4) {
		frequency_table_pointer = frequency_table_iris33;
		bitrate_table_2stage_pointer = bitrate_table_iris33_2stage_fp;
	} else if (codec_input.pipe_num == 2) {
		frequency_table_pointer = frequency_table_iris33_2p;
		bitrate_table_2stage_pointer = bitrate_table_iris33_2p_2stage_fp;
	}
	...
	if ((pixle_count >= fp_pixel_count_bar0) &&
		...
		vsp_hw_min_frequency = frequency_table_pointer[0][2] *
			input_bitrate_fp * 1000;

		if (codec_input.codec == CODEC_AV1)
			vsp_hw_min_frequency = frequency_table_pointer[0][1] *
				input_bitrate_fp * 1000;

		if ((codec_input.codec == CODEC_H264) ||
			(codec_input.codec == CODEC_H264_CAVLC)) {
			vsp_hw_min_frequency = (frequency_table_pointer[0][2] * 1000 +
				(fw_sw_vsp_offset - 1));
			...
			if (codec_input.vsp_vpp_mode == CODEC_VSPVPP_MODE_2S) {
				vsp_hw_min_frequency = vsp_hw_min_frequency +
					(bitrate_table_2stage_pointer[codec][0] *
					fw_sw_vsp_offset - 1);
		...
		if (codec_input.vsp_vpp_mode == CODEC_VSPVPP_MODE_2S) {
			vsp_hw_min_frequency = vsp_hw_min_frequency +
				(bitrate_table_2stage_pointer[codec][bitrate_entry] *
				fw_sw_vsp_offset - 1);
			vsp_hw_min_frequency = DIV_ROUND_UP(vsp_hw_min_frequency,
				(bitrate_table_2stage_pointer[codec][bitrate_entry]) *
					fw_sw_vsp_offset);
...
static u32 calculate_pipe_penalty(struct api_calculation_input codec_input)
{
	...
	u32 pixel_count = 0;
	u32 (*pipe_penalty_pointer)[3];

	if (codec_input.pipe_num == 4)
		pipe_penalty_pointer = pipe_penalty_iris33;
	else if (codec_input.pipe_num == 2)
		pipe_penalty_pointer = pipe_penalty_iris33_2p;

	/* decoder */
	if (codec_input.decoder_or_encoder == CODEC_DECODER) {
		pipe_penalty_codec = pipe_penalty_pointer[0][0];
		...
		if (codec_input.codec == CODEC_AV1) {
			...
			if (pixel_count <= 1920 * 1080)
				pipe_penalty_codec =
					pipe_penalty_pointer[avid_commercial_content + 1][0];
			else if (pixel_count < 3840 * 2160)
				pipe_penalty_codec =
					(pipe_penalty_pointer[avid_commercial_content + 1][0] +
					pipe_penalty_pointer[avid_commercial_content + 1][1]) / 2;
			else if ((pixel_count == 3840 * 2160) ||
				(pixel_count == 4096 * 2160) || (pixel_count == 4096 * 2304))
				pipe_penalty_codec =
					pipe_penalty_pointer[avid_commercial_content + 1][1];
			else if (pixel_count < 7680 * 4320)
				pipe_penalty_codec =
					(pipe_penalty_pointer[avid_commercial_content + 1][1] +
					pipe_penalty_pointer[avid_commercial_content + 1][2]) / 2;
			else
				pipe_penalty_codec =
					pipe_penalty_pointer[avid_commercial_content + 1][2];
		}
	} else {
