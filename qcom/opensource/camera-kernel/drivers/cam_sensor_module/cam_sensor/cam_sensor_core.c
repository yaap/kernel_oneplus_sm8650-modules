	...
	void *arg)
{
	...
	CAM_DBG(CAM_SENSOR, "Header OpCode: 0x%x", csl_packet->header.op_code);
	...
	void *arg)
{
	...
	case CAM_OEM_GET_UNIQUEID: {
		rc = cam_sensor_read_uniqueid(s_ctrl, arg);
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "Read unique id failed");
			goto release_mutex;
		}
		break;
	}
