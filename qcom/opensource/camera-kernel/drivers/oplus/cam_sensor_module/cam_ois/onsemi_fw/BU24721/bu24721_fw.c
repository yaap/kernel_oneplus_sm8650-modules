...
OIS_UBYTE F024_Polling(struct cam_ois_ctrl_t *o_ctrl)
{
    OIS_LONG	u16_i;
    OIS_UBYTE	u16_dat;
    for( u16_i = 1; u16_i <= Poling_times; u16_i ++ ) {
        u16_dat = I2C_OIS_8bit__read(o_ctrl,OIS_status);
        if((u16_dat&0x01) == 1) {
            break;
        }
        Wait(1000);
...
void Erase_flash(struct cam_ois_ctrl_t *o_ctrl)
{
    ...
    I2C_FM_8bit_write(o_ctrl,subaddr_FM_era, 0xE0);
    Wait(2000);//wait 2ms
...
void Prog_flash(struct cam_ois_ctrl_t *o_ctrl)
{
    OIS_UBYTE out[10];
    OIS_ULONG wdata;
    OIS_UWORD addr;
    ...
    out[4] = 0x00;
    out[5] = 0x15;
    I2C_FM_block_write(o_ctrl,out,6);
...
//  *****************************************************
void Flash_sla_change (struct cam_ois_ctrl_t *o_ctrl)
{
    I2C_OIS_8bit_write(o_ctrl,Fla_strl,(FLASH_SLVADR<<1));
    F024_Polling(o_ctrl);
}
//  *****************************************************
//  Write SPI Param, Set FW, DSP data
//  *****************************************************
OIS_LONG bu24_write_flash_fw_data(struct cam_ois_ctrl_t *o_ctrl)
{
    OIS_ULONG		    sts = 1;
    ...
    OIS_ULONG		i, len, mod;
    //*** Access Mode ***//
    I2C_FM_8bit_write(o_ctrl,FM_acc_mode, 0x01);
    //*** Disable Write protect ***//
    I2C_FM_8bit_write(o_ctrl,FM_wri_pro, 0xA5);
#if    1
    ...
    for (i = 0; i < len; i++) {
        ...
        I2C_FM_block_write(o_ctrl,buf,Dat_len + 2);
    ...
    if(mod>0) {
        ...
        I2C_FM_block_write(o_ctrl,buf,mod + 2);
    }
...
void WriteGyroGainToFlash(struct cam_ois_ctrl_t *o_ctrl)
{
    OIS_UWORD X_ori, Y_ori;
    X_ori = I2C_OIS_16bit__read(o_ctrl,Gyro_gain_x);//read 0xF07A
    Y_ori = I2C_OIS_16bit__read(o_ctrl,Gyro_gain_y);//read 0xF07C
    CAM_INFO(CAM_OIS, "[WriteGyroGainToFlash] will write  X_gain= 0x%x  Y_gain= 0x%x",X_ori, Y_ori);

    Update_GyroCalib_to_flash(o_ctrl,0, 0, X_ori, Y_ori);

    X_ori = I2C_OIS_16bit__read(o_ctrl,Gyro_gain_x);//read 0xF07A
    Y_ori = I2C_OIS_16bit__read(o_ctrl,Gyro_gain_y);//read 0xF07C
    CAM_INFO(CAM_OIS, "[WriteGyroGainToFlash] after write  X_gain= 0x%x  Y_gain= 0x%x",X_ori, Y_ori);

}


void Gyro_gain_set(struct cam_ois_ctrl_t *o_ctrl,OIS_UWORD X_gain, OIS_UWORD Y_gain)
{
    OIS_UWORD X_ori, Y_ori;
    ...
    X_ori = I2C_OIS_16bit__read(o_ctrl,Gyro_gain_x);//read 0xF07A
    Y_ori = I2C_OIS_16bit__read(o_ctrl,Gyro_gain_y);//read 0xF07C
    CAM_INFO(CAM_OIS, "[Gyro_gain_set] oldGyorGain  X_gain= 0x%x  X_gain= 0x%x",X_ori, Y_ori);
    CAM_INFO(CAM_OIS, "[Gyro_gain_set] newGyorGain  X_gain= 0x%x  Y_gain= 0x%x",X_gain, Y_gain);
    I2C_OIS_16bit_write(o_ctrl,Gyro_gain_x, X_gain);
    I2C_OIS_16bit_write(o_ctrl,Gyro_gain_y, Y_gain);

    X_ori = I2C_OIS_16bit__read(o_ctrl,Gyro_gain_x);//read 0xF07A
    Y_ori = I2C_OIS_16bit__read(o_ctrl,Gyro_gain_y);//read 0xF07C
    CAM_INFO(CAM_OIS, "[Gyro_gain_set] after write  X_gain= 0x%x  X_gain= 0x%x",X_ori, Y_ori);

}

struct _FACT_ADJ Gyro_offset_cal(struct cam_ois_ctrl_t *o_ctrl) {
    struct _FACT_ADJ resultGyorOffset = { 0x0000, 0x0000};
    OIS_ULONG diff = 0;

    //X offset
    I2C_OIS_8bit_write(o_ctrl,Gyro_offset_lat, 0x00); //0xF088,0x00 select X axis
    Wait(70000);
    F024_Polling(o_ctrl);
    resultGyorOffset.gl_GX_OFS = I2C_OIS_16bit__read(o_ctrl,Gyro_offset_val);//0xF08A
    //Y offset
    I2C_OIS_8bit_write(o_ctrl,Gyro_offset_lat, 0x01); //0xF088,0x01 select Y axis
    Wait(70000);
    F024_Polling(o_ctrl);
    resultGyorOffset.gl_GY_OFS = I2C_OIS_16bit__read(o_ctrl,Gyro_offset_val);//0xF08A
    ...
    //update to register
    I2C_OIS_8bit_write(o_ctrl,Gyro_offset_req,0x00);//0xF09C,0x00 select X axis to update
    I2C_OIS_16bit_write(o_ctrl,Gyro_offset_diff,resultGyorOffset.gl_GX_OFS);//write the X offset value to 0xF09D
    I2C_OIS_8bit_write(o_ctrl,Gyro_offset_req,0x01);//0xF09C,0x01 select Y axis to update
    I2C_OIS_16bit_write(o_ctrl,Gyro_offset_diff,resultGyorOffset.gl_GY_OFS);//write the Y offset value to 0xF09D

    //verify gyro offset
    I2C_OIS_8bit_write(o_ctrl,Gyro_offset_req,0x02);//0xF09C,0x02 to output X diff value
    F024_Polling(o_ctrl);
    diff = I2C_OIS_16bit__read(o_ctrl,Gyro_offset_diff);//read 0xF09D to get the X diff value
    CAM_INFO(CAM_OIS, "[Gyro_offset_cal] Gyro_offset_diff X = 0x%x",diff);
    I2C_OIS_8bit_write(o_ctrl,Gyro_offset_req,0x03);//0xF09C,0x02 to output Y diff value
    F024_Polling(o_ctrl);
    diff = I2C_OIS_16bit__read(o_ctrl,Gyro_offset_diff);//read 0xF09D to get the X diff value
    CAM_INFO(CAM_OIS, "[Gyro_offset_cal] Gyro_offset_diff Y = 0x%x",diff);


    Update_GyroCalib_to_flash(o_ctrl,resultGyorOffset.gl_GX_OFS, resultGyorOffset.gl_GY_OFS,0 ,0);


    return resultGyorOffset;
}

void Update_GyroCalib_to_flash(struct cam_ois_ctrl_t *o_ctrl,OIS_UWORD gyro_X_offset, OIS_UWORD gyro_Y_offset, OIS_UWORD gyro_X_gain, OIS_UWORD gyro_Y_gain)
{
    ...
    I2C_FM_8bit_write(o_ctrl,FM_acc_mode, 0x01);//0xF000,0x01
    for (i = 0; i < (0x200 / 4); i++) {
        // READ 512byte from Start address of Gyro Calibration data on FLASH before ERASE
        rdata = I2C_FM_32bit__read(o_ctrl,FLASH_ADDR_GYRO_CALIB + i);
    ...
    // Disable Write protect
    I2C_FM_8bit_write(o_ctrl,FM_wri_pro, 0xA5);//0xFF0F

    // ERASE 512byte from Start address of Gyro Calibration data on FLASH
    I2C_FM_8bit_write(o_ctrl,FLASH_ADDR_GYRO_CALIB | 0x8000, 0xE0);//0xF800 >> 2
    Wait(2000);//wait 2ms
    ...
    for (i = 0; i < (0x200 / 32); i++) {
        ...
        I2C_FM_block_write(o_ctrl,buf_34, 0x20 + 2);
    }
...
void OIS_mode_set(struct cam_ois_ctrl_t *o_ctrl,OIS_UWORD mode)
{
    if(mode == 0) {
        F024_Polling(o_ctrl);
        I2C_OIS_8bit_write(o_ctrl,OIS_control, OIS_standby);
    } else if(mode == 1) {
        F024_Polling(o_ctrl);
        I2C_OIS_8bit_write(o_ctrl,OIS_control, Servo_on);
    } else if(mode == 2) {
        F024_Polling(o_ctrl);
        I2C_OIS_8bit_write(o_ctrl,OIS_control, OIS_on);
    } else if(mode == 4) {
        F024_Polling(o_ctrl);
        I2C_OIS_8bit_write(o_ctrl,OIS_control, Servo_off);
    }
}
void OIS_manual_set(struct cam_ois_ctrl_t *o_ctrl,OIS_UWORD X_pos, OIS_UWORD Y_pos)
{
    I2C_OIS_8bit_write(o_ctrl,OIS_control, Servo_on);
    F024_Polling(o_ctrl);
    I2C_OIS_16bit_write(o_ctrl,Reg_x_pos, X_pos);
    I2C_OIS_16bit_write(o_ctrl,Reg_y_pos, Y_pos);
    I2C_OIS_8bit_write(o_ctrl,Manu_en, 0x03);
    Wait(50000); //*** wait 50ms for big step ***//
    F024_Polling(o_ctrl);
}
void Update_Gyro_offset_gain_cal_from_flash(struct cam_ois_ctrl_t *o_ctrl)
{
...
*/
    ...
    en_gyrogain_update = I2C_FM_16bit__read(o_ctrl,FLASH_ADDR_GYRO_CALIB + 3); //3D83 check flag data
    CAM_ERR(CAM_OIS, "[Update_Gyro_offset_gain_cal_from_flash] en_gyrogain_update = 0x%x", en_gyrogain_update);
    //*** Notice the Start address of Gyro Calibration data on FLASH ***//
    I2C_OIS_16bit_write(o_ctrl,0xF1E2, FLASH_ADDR_GYRO_CALIB);

    if (en_gyrogain_update == 0xCC01) {
        //*** Gyro Gain Update from FLASH is performed ***//
        I2C_OIS_8bit_write(o_ctrl,0xF1E4, 1);
    } else {
        //*** Gyro Gain Update from FLASH is not performed ***//
        I2C_OIS_8bit_write(o_ctrl,0xF1E4, 0);
    }
}
#define ACCURACY 0x17  //?��8um accuracy
void Circle_test(struct cam_ois_ctrl_t *o_ctrl)
{
    ...
    Wait(100);
    F024_Polling(o_ctrl);
    I2C_OIS_8bit_write(o_ctrl,CIR_thr, ACCURACY);
    F024_Polling(o_ctrl);
    I2C_OIS_8bit_write(o_ctrl,CIR_amp_x, 0x40);  //*** radius 200um ***//
    F024_Polling(o_ctrl);
    I2C_OIS_8bit_write(o_ctrl,CIR_amp_y, 0x40);
    F024_Polling(o_ctrl);
    I2C_OIS_8bit_write(o_ctrl,CIR_sta, 0x00);
    Wait(1000000);
    Wait(1000000);
    Wait(1000000);
    Wait(1000000);
    Wait(1000000);
    F024_Polling(o_ctrl);
    Error = I2C_OIS_8bit__read(o_ctrl,CIR_stu);
    if (Error == 4) {
        //printf("CIR test OK");
    } else {
        Target_x = I2C_OIS_16bit__read(o_ctrl,CIR_tar_x);
        Target_y = I2C_OIS_16bit__read(o_ctrl,CIR_tar_y);
        Error_x = I2C_OIS_16bit__read(o_ctrl,CIR_err_x);
        Error_y = I2C_OIS_16bit__read(o_ctrl,CIR_err_y);
...
void OIS_mag_check(struct cam_ois_ctrl_t *o_ctrl,OIS_UWORD *srv_x, OIS_UWORD *srv_y)
{

    OIS_mode_set(o_ctrl,1);
    Wait(20000);
    F024_Polling(o_ctrl);
    OIS_mode_set(o_ctrl,4);
    F024_Polling(o_ctrl);
    I2C_OIS_8bit_write(o_ctrl,ADC_seq, ADC_x);
    F024_Polling(o_ctrl);
    *srv_x = I2C_OIS_16bit__read(o_ctrl,ADC_val);
    I2C_OIS_8bit_write(o_ctrl,ADC_seq, ADC_y);
    F024_Polling(o_ctrl);
    *srv_y = I2C_OIS_16bit__read(o_ctrl,ADC_val);
    OIS_mode_set(o_ctrl,1);
}
...
void OIS_soft_reset(struct cam_ois_ctrl_t *o_ctrl,OIS_ULONG Prog_ID)
{
    if(Prog_ID!=BU24721_FW_VERSION) {
        I2C_OIS_8bit_write(o_ctrl,OIS_reset_1,0x00);
        Wait(100);
        F024_Polling(o_ctrl);
        I2C_OIS_8bit_write(o_ctrl,OIS_reset_2,0x00);
        Wait(6000);//wait 6ms
        F024_Polling(o_ctrl);
        I2C_OIS_8bit_write(o_ctrl,OIS_release_Standby,0x00);
        Wait(100);
        F024_Polling(o_ctrl);
    }
}

void Gyro_select(struct cam_ois_ctrl_t *o_ctrl,OIS_UBYTE Gyro_sel, bool isTeleOisUseMonitor)
{
    ...
    switch(Gyro_sel) {
        //*** INVENSENSE ICM42631 ***//
    case 0:
        I2C_OIS_8bit_write(o_ctrl,0xf12b, 0x0);

        //I2C_OIS_8bit_write(GYRO_sel_adr, Q_ICM42631);//Enable setting of Qualcomm:ICM42631
        F024_Polling(o_ctrl);
        if (isTeleOisUseMonitor) {
            I2C_OIS_8bit_write(o_ctrl,OIS_SPI, SPI_Monitor);
        } else {
            I2C_OIS_8bit_write(o_ctrl,OIS_SPI, SPI_Master);//Gyro Master/Monitor Switch
        }
        F024_Polling(o_ctrl);
        SPI_Mode_check = I2C_OIS_8bit__read(o_ctrl,OIS_SPI);
        CAM_INFO(CAM_OIS, "SPI_Mode_check:%d ", SPI_Mode_check);
        F024_Polling(o_ctrl);
        //SPI Master setting//
        if(SPI_Mode_check == 0) {
            I2C_OIS_8bit_write(o_ctrl,OIS_gyro_mode, OIS_gyro_con);
            F024_Polling(o_ctrl);
            I2C_OIS_8bit_write(o_ctrl,OIS_gyro_adr, 0x55);
            I2C_OIS_8bit_write(o_ctrl,OIS_gyro_dat, 0x22);
            I2C_OIS_8bit_write(o_ctrl,OIS_gyro_adr, 0x54);
            I2C_OIS_8bit_write(o_ctrl,OIS_gyro_dat, 0x03);
        ...
        //SPI Monitor setting//
            ...
            I2C_OIS_8bit_write(o_ctrl,OIS_gyro_mode, OIS_gyro_on);
            F024_Polling(o_ctrl);
        ...
        //*** ST LSM6DSOQ ***//
    case 1:
        I2C_OIS_8bit_write(o_ctrl,GYRO_sel_adr, M_BMI260);//Enable setting of MTK:BMI260
        F024_Polling(o_ctrl);
        I2C_OIS_8bit_write(o_ctrl,OIS_SPI, SPI_Monitor);//0xF02A,0x04
        F024_Polling(o_ctrl);
        I2C_OIS_8bit_write(o_ctrl,OIS_gyro_mode, OIS_gyro_on);//0xF023,0x00
        F024_Polling(o_ctrl);
        ...
        //*** BOSCH BMI260 ***//
    case 2:
        I2C_OIS_8bit_write(o_ctrl,GYRO_sel_adr, M_BMI260);//Enable setting of MTK:ICM42631
        F024_Polling(o_ctrl);
        I2C_OIS_8bit_write(o_ctrl,OIS_SPI, SPI_Monitor);//0xF02A,0x04
        F024_Polling(o_ctrl);
        I2C_OIS_8bit_write(o_ctrl,OIS_gyro_mode, OIS_gyro_on);//0xF023,0x00
...
void Gyro_data(struct cam_ois_ctrl_t *o_ctrl,OIS_UBYTE count, OIS_UBYTE Gyro_sel)
{
    OIS_UBYTE i;
    ...
    case 1:
        for(i = 0; i < count; i++) {
            gyro_x = I2C_OIS_16bit__read(o_ctrl,0xF0E0);
            gyro_y = I2C_OIS_16bit__read(o_ctrl,0xF0E6);
            Wait(10000);
        }
        break;
    case 2:
        for(i = 0; i < count; i++) {
            gyro_x = I2C_OIS_16bit__read(o_ctrl,0xF0E6);
            gyro_y = I2C_OIS_16bit__read(o_ctrl,0xF0E0);
            Wait(10000);
        }
        break;
    case 3:
        for(i = 0; i < count; i++) {
            gyro_x = I2C_OIS_16bit__read(o_ctrl,0xF0E2);
            gyro_y = I2C_OIS_16bit__read(o_ctrl,0xF0E4);
            Wait(10000);
...
void Boot_err_sla_change (struct cam_ois_ctrl_t *o_ctrl)
{
    OIS_UBYTE out[10];
    I2C_OIS_8bit_write(o_ctrl,OIS_boot_mode,0x55);
    F024_Polling(o_ctrl);

    OIS_soft_reset(o_ctrl,0);

    I2C_OIS_8bit_write(o_ctrl,OIS_boot_mode,0x00);
    F024_Polling(o_ctrl);
    out[0] = 0xF0;
    out[1] = 0x80;
    out[2] = 0xC4;
    ...
    out[4] = 0x44;
    out[5] = 0xE1;
    I2C_OIS_block_write(o_ctrl,out,6);
    F024_Polling(o_ctrl);
...
int Rohm_bu24721_fw_download(struct cam_ois_ctrl_t *o_ctrl)
{
    OIS_ULONG	Prog_ID;
    int ret = 0;

    I2C_OIS_8bit_write(o_ctrl,OIS_release_Standby, 0x00);  //release standby
    ret = F024_Polling(o_ctrl);
    CAM_INFO(CAM_OIS, "polling ret  = %d",ret);

    ret = I2C_OIS_32bit__read(o_ctrl,FW_ID, &Prog_ID);//check 0xF01C program ID
    if (ret < 0) {
    ...
    if(Prog_ID != BU24721_FW_VERSION) {
        if(Prog_ID == 0) {
            Boot_err_sla_change(o_ctrl);
            CAM_ERR(CAM_OIS, "boot err change flash to %x",FLASH_SLVADR<<1);
        } else {
            Flash_sla_change(o_ctrl); //*** change Flash salve address to 0x70 ***//
        }
        bu24_write_flash_fw_data(o_ctrl);

        OIS_soft_reset(o_ctrl,Prog_ID);
...
int bu24721_do_push_center(struct cam_ois_ctrl_t *o_ctrl)
{
    int rc = 0;

    rc = I2C_OIS_8bit_write(o_ctrl,OIS_control, 0x01);
    CAM_INFO(CAM_OIS, "I2C_OIS_8bit_write OIS_control(%x), rc  = %d", OIS_control, rc);
    if (0 == rc)
    {
        Wait(100);
        rc = I2C_OIS_8bit_write(o_ctrl,OIS_status, 0x01);
