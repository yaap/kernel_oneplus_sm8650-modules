...
int WRITE_QTIMER_TO_OIS (struct cam_ois_ctrl_t *o_ctrl)
{
	...
	CAM_DBG(CAM_OIS, "[%s]reg_adr_qtimer = 0x%x",o_ctrl->ois_name,reg_addr_qtimer);
	...
	memset(i2c_write_setting_gl, 0, sizeof(struct cam_sensor_i2c_reg_array)*MAX_DATA_NUM*2);
        while(value>350000  && j<5 ){
	        ...
	        CAM_DBG(CAM_OIS,"qtime_ns: H=0x%x L=0x%x",(uint32_t)((qtime_ns>>32)&0xffffffff),(uint32_t)(qtime_ns&0xffffffff));
		...
		do
		{
			...
			else if(strstr(o_ctrl->ois_name, "dw9786"))
			{
				rc = camera_io_dev_read(&(o_ctrl->io_master_info), 0xE000, (uint32_t *)&ReadVal,
					CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE, false);
				if (rc < 0) {
					CAM_ERR(CAM_OIS, "[DW9786] Read Addr_OIS_STS(0xE000) failed, retry");
				}
				else
				{
					CAM_INFO(CAM_OIS, "[DW9786] Read Addr_OIS_STS(0xE000) = 0x%x", ReadVal);
...
int update_ois_firmware_dw9786(void *arg)
{
	struct cam_ois_ctrl_t *o_ctrl = (struct cam_ois_ctrl_t *)arg;
	int rc = 0;
	CAM_INFO(CAM_OIS, "update_ois_firmware_dw9786");
	o_ctrl->io_master_info.master_type = CCI_MASTER;
	...
	cam_ois_power_down(o_ctrl);
	CAM_INFO(CAM_OIS, "exit update_ois_firmware_dw9786 rc:%d", rc);
	return rc;
}
...
void InitOISResource(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (o_ctrl)
	{
		...
		if (strstr(o_ctrl->ois_name, "bu24721"))
		{
			CAM_INFO(CAM_OIS, "create bu24721 ois download fw thread");
			o_ctrl->ois_downloadfw_thread = kthread_run(update_ois_firmware_bu24721, o_ctrl, o_ctrl->ois_name);
		}

		if (strstr(o_ctrl->ois_name, "sem1217s"))
		{
			CAM_ERR(CAM_OIS, "create sem1217s ois download fw thread for sem1217");
		...
		{
			CAM_ERR(CAM_OIS, "create dw9786 ois download fw thread for dw9786");
...
int32_t oplus_cam_ois_construct_default_power_setting_dw9786(
	struct cam_sensor_power_ctrl_t *power_info)
{
	int rc = 0;
...
int I2C_OIS_8bit_write(struct cam_ois_ctrl_t *o_ctrl,uint32_t addr, uint8_t data)
{
	...
	for(i = 0; i < retry; i++) {
		rc = camera_io_dev_write(&(o_ctrl->io_master_info), &i2c_write);
		if (rc < 0) {
			CAM_ERR(CAM_OIS, "ois type=%d,I2C_OIS_8bit_write 0x%04x failed, retry:%d",o_ctrl->ois_type, addr, i+1);
		} else {
			CAM_DBG(CAM_OIS, "I2C_OIS_8bit_write success ois type=%d,write 0x%04x ,data=%d",o_ctrl->ois_type, addr,data);
			return rc;
		}
	}
	return rc;
...
int I2C_OIS_32bit_write(struct cam_ois_ctrl_t *o_ctrl,uint32_t addr, uint32_t data)
{
	...
	for(i = 0; i < retry; i++) {
		rc = camera_io_dev_write(&(o_ctrl->io_master_info), &i2c_write);
		if (rc < 0) {
			CAM_ERR(CAM_OIS, "ois type=%d,I2C_OIS_32bit_write 0x%04x failed, retry:%d",o_ctrl->ois_type, addr, i+1);
		} else {
            CAM_DBG(CAM_OIS, "I2C_OIS_8bit_write success ois type=%d,write 0x%04x ,data=%d",o_ctrl->ois_type, addr,data);
			return rc;
		}
...
int I2C_FM_8bit_write(struct cam_ois_ctrl_t *o_ctrl,uint32_t addr, uint8_t data)
{
	...
	for(i = 0; i < retry; i++) {
		...
		{
			CAM_DBG(CAM_OIS, "I2C_OIS_8bit_write success ois type=%d,write 0x%04x ,data=%x",o_ctrl->ois_type, addr,data);
			o_ctrl->io_master_info.cci_client->sid = sid_defult;
			return rc;
		}
	}
...
int read_reg_16bit_value_16bit(struct cam_ois_ctrl_t *o_ctrl,unsigned short addr, uint32_t* data)
{
	...
	//CAM_ERR(CAM_OIS, "Read cci%d_Master%d",o_ctrl->io_master_info.cci_client->cci_device,o_ctrl->io_master_info.cci_client->cci_i2c_master);
...
void spi_mode_switch(struct cam_ois_ctrl_t *o_ctrl)
{
	if (strstr(o_ctrl->ois_name, "bu24721"))
	{
		CAM_ERR(CAM_OIS, "spi_mode_switch for bu24721");
		if (o_ctrl->pre_isTeleOisUseMonitor != o_ctrl->isTeleOisUseMonitor)
		{
			I2C_OIS_8bit_write(o_ctrl,0xF020, 01);//ois off/server on
			F024_Polling(o_ctrl);
			if (o_ctrl->pre_isTeleOisUseMonitor)
			{
				CAM_ERR(CAM_OIS, "spi_mode_switch from monitor to master");
				I2C_OIS_8bit_write(o_ctrl,0xF02A, 0x00);//monitor mode
				o_ctrl->isTeleOisUseMonitor = FALSE;
			}
			else
			{
				CAM_INFO(CAM_OIS, "spi_mode_switch from master to monitor");
				I2C_OIS_8bit_write(o_ctrl,0xF02A, 0x04);//master mode
				o_ctrl->isTeleOisUseMonitor = TRUE;
			}
			F024_Polling(o_ctrl);
			I2C_OIS_8bit_write(o_ctrl,0xF023, 0x02);//gyro read off
			F024_Polling(o_ctrl);
			Gyro_select(o_ctrl,Q_ICM42631, o_ctrl->isTeleOisUseMonitor);
		}
	}
...
int oplus_cam_ois_do_push_center(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if(strstr(o_ctrl->ois_name, "bu24721"))
	{
		rc = bu24721_do_push_center(o_ctrl);
...
void Enable_gyro_gain(struct cam_ois_ctrl_t *o_ctrl){
    ...
    CAM_ERR(CAM_OIS, "Enable_gyro_gain when start ois");
