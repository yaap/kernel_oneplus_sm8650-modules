	...
	struct cam_sensor_power_ctrl_t *power_info)
{
	...
	if (strstr(a_ctrl->actuator_name, "sem1217s")||strstr(a_ctrl->actuator_name, "dw9786"))
	{
		power_info->power_setting_size = SEM1217S_POWER_SIZE;
		power_info->power_setting =
			kzalloc(sizeof(struct cam_sensor_power_setting) * SEM1217S_POWER_SIZE,
				GFP_KERNEL);
	}
	else
	{
	...
	if (strstr(a_ctrl->actuator_name, "sem1217s")||strstr(a_ctrl->actuator_name, "dw9786"))
	{
		power_info->power_down_setting_size = SEM1217S_POWER_SIZE;
