...
int32_t oplus_cam_actuator_lock(struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	if (a_ctrl->camera_actuator_shake_detect_enable && a_ctrl->cam_act_last_state == CAM_ACTUATOR_INIT)
	{
		...
		else if(strstr(a_ctrl->actuator_name, "dw9786"))
		{
		...
		{
			rc = oplus_cam_actuator_ram_write_word(a_ctrl, 0xE000, 0x0000);
			CAM_DBG(CAM_ACTUATOR, "SDS set reg: 0xE000, data: 0x0000, rc = %d", rc);
			msleep(10);
			rc = oplus_cam_actuator_ram_write_word(a_ctrl, 0xE000, 0x0001);
			CAM_DBG(CAM_ACTUATOR, "SDS set reg: 0xE001, data: 0x0001, rc = %d", rc);
			msleep(5);
			rc = oplus_cam_actuator_ram_write_word(a_ctrl, 0xE2FC, 0xAC1E);
			CAM_DBG(CAM_ACTUATOR, "SDS set reg: 0xE2FC, data: 0xAC1E, rc = %d", rc);
			rc = oplus_cam_actuator_ram_write_word(a_ctrl, 0xE164, 0x0008);
			CAM_DBG(CAM_ACTUATOR, "SDS set reg: 0xE164, data: 0x0008, rc = %d", rc);
			rc = oplus_cam_actuator_ram_write_word(a_ctrl, 0xE2FC, 0x0000);
			CAM_DBG(CAM_ACTUATOR, "SDS set reg: 0xE2FC, data: 0x0000, rc = %d", rc);
			msleep(1);
			rc = oplus_cam_actuator_ram_write_word(a_ctrl, 0xE004, 0x0001);
			CAM_DBG(CAM_ACTUATOR, "SDS set reg: 0xE004, data: 0x0001, rc = %d", rc);
			msleep(10);
			rc = oplus_cam_actuator_ram_write_word(a_ctrl, 0xB026, 0x0001);
			CAM_DBG(CAM_ACTUATOR, "SDS set reg: 0xB026, data: 0x0001, rc = %d", rc);
			rc = oplus_cam_actuator_ram_write_word(a_ctrl, 0xB024, 0x0002);
