...
#define MAX_UNIQUE_ID_LENGTH 24
...
int cam_ftm_power_down(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->sensordata->slave_info.sensor_id == 0x0615||
		...
		s_ctrl->sensordata->slave_info.sensor_id == 0x0906 ||
		s_ctrl->sensordata->slave_info.sensor_id == 0x0471 ||
...
int cam_ftm_power_up(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	else if (s_ctrl->sensordata->slave_info.sensor_id == 0xa18a)
	{
		...
		&vendor_id,s_ctrl->sensordata->id_info.sensor_addr_type,
		CAMERA_SENSOR_I2C_TYPE_BYTE,FALSE);
		CAM_ERR(CAM_SENSOR, "IMX06A vendor id expect 0x%x read  0x%x",s_ctrl->sensordata->id_info.sensor_id,vendor_id);
		if(s_ctrl->sensordata->id_info.sensor_id <= 15 && vendor_id <= 15) {
			...
			sensor_setting.reg_setting = sensor_init_settings.imx06A_setting_MP.reg_setting;
			sensor_setting.addr_type = sensor_init_settings.imx06A_setting_MP.addr_type;
			sensor_setting.data_type = sensor_init_settings.imx06A_setting_MP.data_type;
			sensor_setting.size = sensor_init_settings.imx06A_setting_MP.size;
			sensor_setting.delay = sensor_init_settings.imx06A_setting_MP.delay;
			rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_setting);
		}
	}
	...
	else if (s_ctrl->sensordata->slave_info.sensor_id == 0x3109)
	{
		oplus_shift_sensor_mode(s_ctrl);
		CAM_ERR(CAM_SENSOR, "FTM sensor setting 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
		sensor_setting.reg_setting = sensor_init_settings.s5k3p9_setting.reg_setting;
		sensor_setting.addr_type = sensor_init_settings.s5k3p9_setting.addr_type;
	...
	else if (s_ctrl->sensordata->slave_info.sensor_id == 0x0906)
	{
		oplus_shift_sensor_mode(s_ctrl);
		CAM_ERR(CAM_SENSOR, "FTM sensor setting 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
		sensor_setting.reg_setting = sensor_init_settings.imx906_setting.reg_setting;
		sensor_setting.addr_type = sensor_init_settings.imx906_setting.addr_type;
		sensor_setting.data_type = sensor_init_settings.imx906_setting.data_type;
		sensor_setting.size = sensor_init_settings.imx906_setting.size;
		sensor_setting.delay = sensor_init_settings.imx906_setting.delay;
		rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_setting);
	}
	else if (s_ctrl->sensordata->slave_info.sensor_id == 0x0471)
	{
		oplus_shift_sensor_mode(s_ctrl);
		CAM_ERR(CAM_SENSOR, "FTM sensor setting 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
		sensor_setting.reg_setting = sensor_init_settings.imx471_setting.reg_setting;
		sensor_setting.addr_type = sensor_init_settings.imx471_setting.addr_type;
		sensor_setting.data_type = sensor_init_settings.imx471_setting.data_type;
		sensor_setting.size = sensor_init_settings.imx471_setting.size;
		sensor_setting.delay = sensor_init_settings.imx471_setting.delay;
		rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_setting);
	}
	else if (s_ctrl->sensordata->slave_info.sensor_id == 0x002B)
	{
		oplus_shift_sensor_mode(s_ctrl);
		CAM_ERR(CAM_SENSOR, "FTM sensor setting 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
		sensor_setting.reg_setting = sensor_init_settings.ov02b_setting.reg_setting;
		sensor_setting.addr_type = sensor_init_settings.ov02b_setting.addr_type;
		sensor_setting.data_type = sensor_init_settings.ov02b_setting.data_type;
		sensor_setting.size = sensor_init_settings.ov02b_setting.size;
		sensor_setting.delay = sensor_init_settings.ov02b_setting.delay;
		rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_setting);
	}
	else if (s_ctrl->sensordata->slave_info.sensor_id == 0x0896)
	{
		oplus_shift_sensor_mode(s_ctrl);
		CAM_ERR(CAM_SENSOR, "FTM sensor setting 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
		sensor_setting.reg_setting = sensor_init_settings.imx896_setting.reg_setting;
	...
	else if (s_ctrl->sensordata->slave_info.sensor_id == 0x32e2)
	{
		oplus_shift_sensor_mode(s_ctrl);
		CAM_ERR(CAM_SENSOR, "FTM sensor setting 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
		sensor_setting.reg_setting = sensor_init_settings.gc32e2_setting.reg_setting;
		sensor_setting.addr_type = sensor_init_settings.gc32e2_setting.addr_type;
...
int cam_sensor_match_id_oem(struct cam_sensor_ctrl_t *s_ctrl,uint32_t chip_id)
{
	...
	if(chip_id == CAM_IMX06A_SENSOR_ID){
		...
		if(rc == 0)
		{
			/*if vendor_id id  < 16(0x10),it is cut0.9~cut7.0 module else if vendor_id id  >= 16(0x10),it is MP module*/
			if(vendor_id > 15){
				if(s_ctrl->sensordata->id_info.sensor_id > 15)
				{
					return 0;
				}
				else
	...
	else if (chip_id == CAM_IMX471_SENSOR_ID)
	{
...
int cam_sensor_read_uniqueid(struct cam_sensor_ctrl_t *s_ctrl, void *arg)
{
	int rc = 0;
	int i = 0;
	...
	{
		if (regsettings[i].operation == CCI_WRITE)
		{
			rc = SensorRegWrite(s_ctrl,regsettings[i].reg_addr,regsettings[i].reg_data);
		}
		else if (regsettings[i].operation == CCI_READ)
		{
			rc = camera_io_dev_read(
		...
		{
			CAM_ERR(CAM_EEPROM, "regsettingsUnsupported operation %d", regsettings[i].operation);
		}
		if(regsettings[i].delay > 0)
		{
			mdelay(regsettings[i].delay);
		}
		if(rc < 0){
			CAM_ERR(CAM_SENSOR, "Error on 0x%x, operation",
							regsettings[i].reg_addr, regsettings[i].operation);
			break;
		}
	}
	CAM_ERR(CAM_SENSOR, " uniqueid %s",uniqueid);
	if (copy_to_user((void __user *) cmd->handle + cmd->size, uniqueid, MAX_UNIQUE_ID_LENGTH))
	{
		CAM_ERR(CAM_SENSOR, "uniqueid copy_to_user failed ");
		rc = -1;
	}
	...
	if (rc < 0) {
		CAM_ERR(CAM_SENSOR, "Power down failed for %s ", s_ctrl->sensor_name);
		return rc;
	}
	return rc;
}
...
int cam_sensor_start(struct cam_sensor_ctrl_t *s_ctrl, void *arg)
{
	...
	s_ctrl->is_surpport_wr_burst = m_support_burst;
