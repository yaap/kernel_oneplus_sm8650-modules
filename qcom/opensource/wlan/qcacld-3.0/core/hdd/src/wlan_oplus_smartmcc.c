...
int handle_private_miracast_cmd(struct wlan_hdd_link_info *link_info, int sta_quota);
...
int drv_cmd_smartmcc_get_fw_update_quota(struct wlan_hdd_link_info *link_info,
...
bool check_private_miracast_cmd(uint8_t *sub_command, int *sta_quota)
{
    ...
    if (sscanf(sub_command, "%d %d", &sub_command_head, sta_quota) != 2) {
        oplus_debug("check_private_miracast_cmd fail: cmd format check fail");
        return false;
    }
    if (sub_command_head != -1) {
        oplus_debug("check_private_miracast_cmd fail: sub cmd head check fail");
        return false;
    }

    oplus_debug("private sub cmd:%s, sta_quota:%d", sub_command, *sta_quota);
...
int handle_private_miracast_cmd(struct wlan_hdd_link_info *link_info, int sta_quota)
{
    if (sta_quota == -1) {
        ...
        return wlan_hdd_set_mas(link_info->adapter, sta_quota);
    }
}
...
* @link_info->adapter: pointer to link_info->adapter on which request is received
