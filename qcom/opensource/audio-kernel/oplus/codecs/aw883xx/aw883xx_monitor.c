/*
 * aw883xx_monitor.c   aw883xx codec driver
 *
...
{
	int ret = -1;
	struct aw_dsp_monitor_desc *dsp_monitor_desc = &aw_dev->monitor_desc.dsp_monitor_desc;

	ret = aw_dev->ops.aw_dsp_write(aw_dev, dsp_monitor_desc->extn_temp_reg,
				sys_temp, dsp_monitor_desc->extn_temp_reg_type);
	if (ret < 0) {
...
static void aw_monitor_set_gain(struct aw_device *aw_dev, uint16_t gain)
{
	...
	aw_dev_info(aw_dev->dev, "set gain: 0x%x", compared_vol);
}
...
{
	struct aw_monitor_cfg *monitor_cfg = &aw_dev->monitor_desc.monitor_cfg;
	struct aw_vmax_desc *desc = &aw_dev->monitor_desc.vmax_desc;
	bool status = 0;
	uint16_t vmax_set = 0;
	int ret = -1;
	...
	if (!status) {
		aw_dev_err(aw_dev->dev, "pa status error");
		return;
	}
...
static void aw_monitor_check_sysint(struct aw_device *aw_dev)
{
	...
	ret = aw883xx_device_params(aw_dev, AW_DEV_INT_PARAMS,
			(void *)&sysint, sizeof(sysint), AW_GET_DEV_PARAMS);
	if (ret < 0) {
...
static int aw_monitor_work(struct aw_device *aw_dev)
{
	...
	if (aw883xx_cali_get_cali_status(&aw_dev->cali_desc)) {
...
static void aw_sw_monitor_work_func(struct work_struct *work)
{
	uint32_t hmute_st = 0;
	int ret = -1;
	...
	if ((monitor_cfg->monitor_status == AW_MON_CFG_OK) &&
		...
		ret = aw883xx_device_params(aw_dev, AW_DEV_HMUTE_PARAMS,
			(void *)&hmute_st, sizeof(hmute_st), AW_GET_DEV_PARAMS);
		if (ret < 0) {
			aw_dev_err(aw_dev->dev, "get_hmute_status failed");
			return;
		}
...
static int aw_dsp_monitor_work(struct aw_device *aw_dev)
{
	int32_t sys_temp = 0;
...
static void aw_dsp_monitor_work_func(struct work_struct *work)
{
	uint32_t hmute_st = 0;
	int ret = -1;
	struct aw_device *aw_dev  = container_of(work,
		struct aw_device, monitor_desc.dsp_monitor_work.work);
	struct aw883xx *aw883xx = (struct aw883xx *)aw_dev->private_data;
	struct aw_monitor_desc *monitor = &aw_dev->monitor_desc;
	...
	if (hmute_st == AW_DEV_HMUTE_DISABLE) {
		aw_dsp_monitor_work(aw_dev);
		queue_delayed_work(aw883xx->work_queue,
					&monitor->dsp_monitor_work,
					msecs_to_jiffies(monitor->dsp_monitor_delay));
	}
}

static void aw_dsp_monitor_start(struct aw_monitor_desc *monitor_desc)
{
	struct aw_device *aw_dev = container_of(monitor_desc,
			struct aw_device, monitor_desc);
	struct aw883xx *aw883xx = (struct aw883xx *)aw_dev->private_data;

	if (monitor_desc->dsp_temp_flag == AW_EXTERNAL_TEMP) {
		queue_delayed_work(aw883xx->work_queue,
				&monitor_desc->dsp_monitor_work, 0);
	} else {
...
static void aw_sw_monitor_start(struct aw_monitor_desc *monitor_desc)
{
	struct aw_device *aw_dev = container_of(monitor_desc,
			struct aw_device, monitor_desc);
	struct aw883xx *aw883xx = (struct aw883xx *)aw_dev->private_data;
...
void aw883xx_monitor_start(struct aw_monitor_desc *monitor_desc)
{
	...
	if (monitor_desc->hw_monitor_en == AW_MONITOR_ENABLE) {
		aw_dev_info(aw_dev->dev, "hardware monitor is enable");
		return;
	}

	aw_dev_info(aw_dev->dev, "sortware monitor is enable");
	return aw_sw_monitor_start(monitor_desc);
}
...
int aw883xx_monitor_stop(struct aw_monitor_desc *monitor_desc)
{
	...
	cancel_delayed_work_sync(&monitor_desc->dsp_monitor_work);

	return 0;
}

static void aw_monitor_get_hw_monitor_st(struct aw_monitor_desc *monitor_desc)
{
	int ret = -1;
	uint16_t reg_data = 0;
	struct aw_device *aw_dev = container_of(monitor_desc,
			struct aw_device, monitor_desc);
	struct aw_hw_monitor_desc *hw_monitor_desc = &monitor_desc->hw_monitor_desc;

	if (hw_monitor_desc->reg == AW_REG_NONE)
		return;

	ret = aw_dev->ops.aw_reg_read(aw_dev, hw_monitor_desc->reg, &reg_data);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "read 0x%02x failed", hw_monitor_desc->reg);
		return;
	}

	if ((reg_data & (~hw_monitor_desc->bop_mask)) &&
			((reg_data & (~hw_monitor_desc->vol_mask)) ||
			(reg_data & (~hw_monitor_desc->ipeak_mask))))
		monitor_desc->hw_monitor_en = AW_MONITOR_ENABLE;
	else
		monitor_desc->hw_monitor_en = AW_MONITOR_DISABLE;
}

static void aw_monitor_get_dsp_monitor_st(struct aw_monitor_desc *monitor_desc)
{
	int ret = -1;
	uint32_t temp_reg_value = 0;
	uint32_t volt_reg_value = 0;
	uint32_t dsp_volt_en = 0;
	uint32_t dsp_temp_en = 0;
	uint32_t extern_temp_en = 0;
	struct aw_device *aw_dev = container_of(monitor_desc,
			struct aw_device, monitor_desc);
	struct aw_dsp_monitor_desc *dsp_monitor_desc = &monitor_desc->dsp_monitor_desc;

	if ((dsp_monitor_desc->dsp_temp_reg == AW_REG_NONE) ||
		(dsp_monitor_desc->dsp_volt_reg == AW_REG_NONE))
		return;

	ret = aw_dev->ops.aw_dsp_read(aw_dev, dsp_monitor_desc->dsp_temp_reg,
			&temp_reg_value, dsp_monitor_desc->dsp_temp_reg_type);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "read 0x%04x failed", dsp_monitor_desc->dsp_temp_reg);
		return;
	}

	ret = aw_dev->ops.aw_dsp_read(aw_dev, dsp_monitor_desc->dsp_volt_reg,
			&volt_reg_value, dsp_monitor_desc->dsp_volt_reg_type);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "read 0x%04x failed", dsp_monitor_desc->dsp_temp_reg);
		return;
	}

	dsp_temp_en = temp_reg_value & (~dsp_monitor_desc->dsp_temp_en_mask);
	extern_temp_en = temp_reg_value & (~dsp_monitor_desc->extn_temp_en_mask);
	dsp_volt_en = volt_reg_value & (~dsp_monitor_desc->dsp_volt_en_mask);

	if (extern_temp_en)
		monitor_desc->dsp_temp_flag = AW_EXTERNAL_TEMP;
	else
		monitor_desc->dsp_temp_flag = AW_INTERNAL_TEMP;

	if (dsp_temp_en || dsp_volt_en)
		monitor_desc->dsp_monitor_en = AW_MONITOR_ENABLE;
	else
		monitor_desc->dsp_monitor_en = AW_MONITOR_DISABLE;

}

static void aw_monitor_set_init_vmax(struct aw_monitor_desc *monitor_desc)
{
	int ret = -1;
	struct aw_vmax_desc *desc = &monitor_desc->vmax_desc;
	struct aw_device *aw_dev = container_of(monitor_desc,
			struct aw_device, monitor_desc);

	ret = aw_dev->ops.aw_dsp_write(aw_dev, desc->dsp_reg, desc->init_vmax, desc->data_type);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "set vmax failed");
		return;
	}
}

static void aw_monitor_get_init_vmax(struct aw_monitor_desc *monitor_desc)
{
	int ret = -1;
	unsigned int vmax = 0;
	struct aw_vmax_desc *desc = &monitor_desc->vmax_desc;
...
void aw883xx_monitor_update_st(struct aw_monitor_desc *monitor_desc, monitor_update_t type)
{
	...
	switch (type) {
	case AW_GET_INIT_VMAX:
		aw_monitor_get_init_vmax(monitor_desc);
		break;
	case AW_SET_INIT_VMAX:
		aw_monitor_set_init_vmax(monitor_desc);
		break;
	case AW_DSP_MONITOR_ST:
		aw_monitor_get_dsp_monitor_st(monitor_desc);
		break;
	case AW_HW_MONITOR_ST:
		aw_monitor_get_hw_monitor_st(monitor_desc);
		break;
	default:
...
static void aw_monitor_parse_dt(struct aw_device *aw_dev)
{
	...
	ret = of_property_read_u32(np, "dsp_monitor_delay", &monitor_delay);
	if (ret < 0) {
		aw_dev_info(aw_dev->dev,
			"read hw-monitor-delay failed, set deafult value:[%d]ms",
			aw_dev->monitor_desc.dsp_monitor_delay);
	} else {
		aw_dev_info(aw_dev->dev,
			"parse hw-monitor-delay:[%d]", monitor_delay);
		aw_dev->monitor_desc.dsp_monitor_delay = monitor_delay;
	}
}
...
void aw883xx_monitor_init(struct aw_monitor_desc *monitor_desc)
{
	...
	INIT_DELAYED_WORK(&monitor_desc->delay_work, aw_sw_monitor_work_func);
	INIT_DELAYED_WORK(&monitor_desc->dsp_monitor_work, aw_dsp_monitor_work_func);
