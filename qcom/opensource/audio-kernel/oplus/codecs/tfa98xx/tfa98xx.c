...
#define EVT1_PCB_VERSION 24
#define EVT2_PCB_VERSION 25
	...
	if (error) {
		...
		*speakerImpedance = tfa->mohm[0];
		if (TFA_GET_BF(tfa, PWDN) != 0) {
			error = tfa98xx_powerdown(tfa, 0);  //leave power off
			pr_err("%s tfa98xx_powerdown (error = %d)\n", __func__, error);
		}
...
static ssize_t tfa98xx_dbgfs_f0_range_read(struct file *file,
				char __user *user_buf, size_t count,
				loff_t *ppos)
{
#ifdef CONFIG_DEBUG_FS
	...
	ret = snprintf(str, PAGE_SIZE, "F0, Min:%d Hz, Max:%d Hz\n",
...
static const struct proc_ops tfa98xx_dbgfs_f0_range_fops = {
	.proc_open = simple_open,
	.proc_read = tfa98xx_dbgfs_f0_range_read,
};
#endif /* OPLUS_ARCH_EXTENDS */
...
static void tfa98xx_debug_init(struct tfa98xx *tfa98xx, struct i2c_client *i2c)
{
	...
	#ifdef OPLUS_ARCH_EXTENDS
	debugfs_create_file("f0_range", S_IRUGO, tfa98xx->dbg_dir,
				i2c, &tfa98xx_dbgfs_f0_range_fops);
	#endif /* OPLUS_ARCH_EXTENDS */
	...
	proc_create_data("f0_range", S_IRUGO, tfa98xx->dbg_dir,
					&tfa98xx_dbgfs_f0_range_fops, i2c);
	...
	unsigned char *result_buffer)
{
	...
	//mutex_unlock(&tfa98xx->dsp_lock);
	if (ret) {
		pr_err("[0x%x] dsp_msg_read error: %d, ret %d \n",
			tfa->slave_address, error, ret);
		return Tfa98xx_Error_Fail;
	}
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	pcb_ver = get_PCB_Version();
	pr_info("loaded pcb_ver = %d\n", pcb_ver);
	if (tfa98xx->tfa->is_need_reverse == 1) {
		if (pcb_ver != 0 && pcb_ver < EVT2_PCB_VERSION) {
			new_fw_name = fw_name_reverse;
		}
	} else if (tfa98xx->tfa->is_need_reverse == 2) {
		if (pcb_ver != 0 && pcb_ver < EVT1_PCB_VERSION) {
			new_fw_name = fw_name_reverse;
		}
	}
#endif /* OPLUS_ARCH_EXTENDS */
	...
	if (tfa_is_cold(tfa98xx->tfa) == 0) {
		pr_info("Notice:device 0x%.2x still warm\n",
			tfa98xx->i2c->addr);
...
#endif
{
	...
	/*2024/08/16, used to distinguish different project pcb version*/
	ret = of_property_read_u32(i2c->dev.of_node, "tfa_need_reverse", &tfa98xx->tfa->is_need_reverse);
	if (ret) {
