...
{
	if (!wait_for_completion_timeout(&switch_priv->resume_ack, msecs_to_jiffies(RESUME_TIMEDOUT_MS))) {
...
static void typec_switch_shutdown(struct i2c_client *i2c) {
	...
	//2024/3/13, Resolving null pointer issues caused by probe failures
	if (!switch_priv) {
		pr_err("switch_priv is null\n");
		return;
	}
