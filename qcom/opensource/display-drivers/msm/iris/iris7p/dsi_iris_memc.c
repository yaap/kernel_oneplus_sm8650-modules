...
void iris_frc_reg_add_i7p(u32 addr, u32 val, bool last)
{
	...
	if (iris_memc_cmd_payload_count >= (IRIS_MEMC_CMD_PAYLOAD_LENGTH - 2)) {
		iris_ocp_write_mult_vals(iris_memc_cmd_payload_count, iris_memc_cmd_payload);
		iris_memc_cmd_payload_count = 0;
		IRIS_LOGI("re_add:%x val:%x last:%x send payload reg settings", addr, val, last);
	}
	...
	if (iris_memc_cmd_payload_count < IRIS_MEMC_CMD_PAYLOAD_LENGTH) {
		...
		*payload = val;
		...
		IRIS_LOGE("re_add:%x val:%x  last:%x payload buffer not enough!", addr, val, last);
	}
}
...
void iris_frc_mif_reg_set_i7p(void)
{
	...
	phase_ratio = pcfg->memc_info.panel_fps * 100 / 60;
	...
	if (pcfg->memc_info.panel_fps != 60) {
		/* according to the panel timing ratio to config PHASE_DELTA_TH0~6*/
		val_frcc_fk_ctrl_3 = BITS_SET(val_frcc_fk_ctrl_3, 6, 16, DIV_ROUND_UP(62 * 100, phase_ratio));
		val_frcc_fk_ctrl_3 = BITS_SET(val_frcc_fk_ctrl_3, 6, 24, DIV_ROUND_UP(60 * 100, phase_ratio));
		val_frcc_fk_ctrl_4 = BITS_SET(val_frcc_fk_ctrl_4, 6, 0, DIV_ROUND_UP(45 * 100, phase_ratio));

		val_frcc_fk_ctrl_5 = 0;
		val_frcc_fk_ctrl_5 = val_frcc_fk_ctrl_5 | (DIV_ROUND_UP(20 * 100, phase_ratio) << 18 |
							(DIV_ROUND_UP(26 * 100, phase_ratio) << 12) |
							(DIV_ROUND_UP(30 * 100, phase_ratio) << 6) |
							(DIV_ROUND_UP(40 * 100, phase_ratio)));
	}
	...
	if (iris_low_latency_mode_get_i7p() == ULTRA_LT_MODE) {
		...
		/* phase_mapping_en=1 & DEFAULT_MTN_LVL = 4 on ULL mode for game scenechange*/
...
void iris_fi_reg_set_i7p(void)
{
	...
	if (pcfg->frc_demo_window == DEMO_WINDOW_LEFT) {
		...
		iris_frc_reg_add_i7p(IRIS_FI_ADDR + FI_DEMO_WIN_SIZE_1,
			frc_setting->mv_vres << 16 | frc_setting->mv_hres, 0);
	} else if (pcfg->frc_demo_window == DEMO_WINDOW_FULL_OEM ||
			    pcfg->frc_demo_window == DEMO_WINDOW_FULL) {
		iris_frc_reg_add_i7p(IRIS_FI_ADDR + FI_DEMO_WIN_SIZE_0,  0, 0);
		iris_frc_reg_add_i7p(IRIS_FI_ADDR + FI_DEMO_WIN_SIZE_1,
			frc_setting->mv_vres << 16 | frc_setting->mv_hres, 0);
	}
	...
	if (pcfg->memc_info.memc_mode == MEMC_SINGLE_GAME_ENABLE) {
		iris_frc_reg_add_i7p(IRIS_FI_ADDR + FI_OSD0_BR, (frc_setting->mv_vres / 2 + 5) << 16 |
			(frc_setting->mv_hres / 2 + 5), 0);
		iris_frc_reg_add_i7p(IRIS_FI_ADDR + FI_OSD0_TL, (frc_setting->mv_vres / 2 - 5) << 16 |
			(frc_setting->mv_hres / 2 - 5), 0);
	}
...
void iris_frc_setting_update_i7p(void)
{
	...
	IRIS_LOGI("frc setting: disp-0x%04x%04x@%d, fw-%d, buf-%d@%x frc_vfr:%d",
...
void iris_demo_wnd_set_i7p(void)
{
	...
	iris_memc_cmd_payload_init_i7p();
	iris_frc_reg_add_i7p(IRIS_FI_ADDR + FI_DEMO_MODE_CTRL, 0x4000 |
			pcfg->frc_label << 7 |
			pcfg->demo_border << 4 |
			pcfg->demo_enable << 1 |
			pcfg->demo_color_en
			, 0);
	iris_frc_reg_add_i7p(IRIS_FI_ADDR + FI_DEMO_WIN_SIZE_0,
			pcfg->demo_cur_start_x | pcfg->demo_cur_start_y << 16, 0);
	iris_frc_reg_add_i7p(IRIS_FI_ADDR + FI_DEMO_WIN_SIZE_1,
			pcfg->demo_cur_end_x | pcfg->demo_cur_end_y << 16, 0);
	iris_frc_reg_add_i7p(IRIS_FI_ADDR + FI_DEMO_YUV_VALUE, pcfg->demo_color, 0);
	iris_frc_reg_add_i7p(IRIS_FI_ADDR + FI_SHDW_CTRL, 1, 1);
	iris_memc_cmd_payload_send_i7p();
}
...
void iris_memc_func_init_i7p(struct iris_memc_func *memc_func)
{
	...
	memc_func->not_allow_off_primary = NULL;
	memc_func->not_allow_off_secondary = NULL;
	memc_func->blending_enable = NULL;
}
