...
static int aw883xx_dev_hw_reset(struct aw883xx *aw883xx)
{
...
int aw883xx_device_params(struct aw_device *aw_dev, dev_params_t params_type, void *data, uint32_t size, params_option_t params_ops)
{
	if (params_ops == AW_SET_DEV_PARAMS) {
		return aw_dev_set_params(aw_dev, params_type, (uint32_t *)data, size);
	} else if (params_ops == AW_GET_DEV_PARAMS) {
		return aw_dev_get_params(aw_dev, params_type, (uint32_t *)data, size);
	} else {
...
static void aw883xx_parse_f0_range_dt(struct aw_device *aw_dev)
{
	int ret = -1;
	uint32_t f0_max = 0;
	uint32_t f0_min = 0;
	...
	ret = of_property_read_u32(np, "f0-min", &f0_min);
	if (ret < 0) {
        aw_dev->f0_range.f0_min = AW_CALI_F0_DEFAULT_MIN;
		aw_dev_info(aw_dev->dev, "read f0-min value failed, use default");
	} else {
		aw_dev_info(aw_dev->dev,"parse f0-min:[%d]", f0_min);
		aw_dev->f0_range.f0_min = f0_min;
	}

	ret = of_property_read_u32(np, "f0-max", &f0_max);
	if (ret < 0) {
        aw_dev->f0_range.f0_max = AW_CALI_F0_DEFAULT_MAX;
		aw_dev_info(aw_dev->dev, "read f0-max failed, use default");
	} else {
		aw_dev_info(aw_dev->dev,"parse f0-max:[%d]", f0_max);
		aw_dev->f0_range.f0_max = f0_max;
	}
}
...
static int aw_device_parse_dt(struct aw_device *aw_dev)
{
	...
	aw883xx_parse_f0_range_dt(aw_dev);
