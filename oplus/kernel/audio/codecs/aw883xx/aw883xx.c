...
/*2023/05/16, Add for smartpa err feedback.*/
...
#define AW883XX_DRIVER_VERSION "v1.5.0.1"
...
const unsigned char fb_regs_aw88394[] = {0x02, 0x04, 0x05, 0x06, 0x07, 0x09, 0x0a, 0x0b, 0x0c, 0x10, 0x12, 0x13, 0x14, 0x60, 0x61};

static bool g_chk_err = false;

static int aw883xx_check_status_reg(struct aw883xx *aw883xx)
{
	...
	if ((aw883xx->last_fb !=0) && ktime_before(ktime_get(), ktime_add_ms(aw883xx->last_fb, MM_FB_KEY_RATELIMIT_1MIN))) {
		return 0;
	}

	ret = aw883xx_i2c_read(aw883xx, AW883XX_STATUS_REG, &reg_val);
	if (ret < 0) {
		offset = strlen(info);
		scnprintf(info + offset, sizeof(info) - offset - 1, \
				"aw883xx SPK%u:failed to read regs 0x%x, ret=%d,", \
				aw883xx->index + 1, AW883XX_STATUS_REG, ret);
		aw_dev_info(aw883xx->dev, "i2c read error, ret=%d", ret);
	} else {
		aw_dev_info(aw883xx->dev, "read reg[0x%x]=0x%x", AW883XX_STATUS_REG, reg_val);
		if ((AW883XX_STATUS_NORMAL_VALUE & AW883XX_STATUS_CHECK_MASK) != (reg_val & AW883XX_STATUS_CHECK_MASK)) {
			offset = strlen(info);
			scnprintf(info + offset, sizeof(info) - offset - 1, \
					"aw883xx SPK%u:reg[0x%x]=0x%x,", \
					aw883xx->index + 1, AW883XX_STATUS_REG, reg_val);
			...
			if (aw883xx->aw_pa && (AW883XX_PID_2049 == aw883xx->aw_pa->chip_id)) {
				for (i = 0; i < sizeof(fb_regs_aw88394); i++) {
					ret = aw883xx_i2c_read(aw883xx, fb_regs_aw88394[i], &reg_val);
					if (ret < 0) {
						break;
					} else {
						offset = strlen(info);
	...
	if ((offset > 0) && (offset < MM_KEVENT_MAX_PAYLOAD_SIZE)) {
		...
		aw883xx->last_fb = ktime_get();
		aw_dev_info(aw883xx->dev, "fd_buf=%s", fd_buf);
	}

	return 1;
}

static int aw883xx_set_check_feedback(struct snd_kcontrol *kcontrol,
...
static int aw883xx_get_check_feedback(struct snd_kcontrol *kcontrol,
						struct snd_ctl_elem_value *ucontrol)
{
...
static char const *aw883xx_check_feedback_text[] = {"Off", "On"};
static const struct soc_enum aw883xx_check_feedback_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(aw883xx_check_feedback_text), aw883xx_check_feedback_text);

static const struct snd_kcontrol_new aw883xx_check_feedback[] = {
	SOC_ENUM_EXT("AW_CHECK_FEEDBACK", aw883xx_check_feedback_enum,
			   aw883xx_get_check_feedback, aw883xx_set_check_feedback),
};
...
static void aw883xx_start_pa(struct aw883xx *aw883xx)
{
	...
	if (speaker_mute_control) {
		aw_dev_info(aw883xx->dev, "pa is force mute");
		return;
	}
...
/*2023/05/16, Add for smartpa err feedback.*/
	    ...
	    if (g_chk_err) {
                        aw883xx->last_fb = 0;
			aw883xx_check_status_reg(aw883xx);
			g_chk_err = false;
		}
        }
...
static int aw_pa_control(struct list_head *dev_list, bool flag)
{
	int ret;
	...
	list_for_each(pos, dev_list) {
		...
		if (flag) {
			ret = aw883xx_device_start(local_dev);
			if (ret < 0) {
				aw_dev_err(local_dev->dev, "on pa failed");
				return ret;
			}
		} else {
			ret = aw883xx_device_stop(local_dev);
			if (ret < 0) {
				aw_dev_err(local_dev->dev, "off pa failed");
				return ret;
			}
		}
	}
...
int aw883xx_set_pa_ctl(struct aw883xx *aw883xx, bool flag)
{
        int ret = 0;
	struct list_head *dev_list = NULL;

	aw_dev_info(aw883xx->dev,"pa on enter");
		...
		ret = aw_pa_control(dev_list, flag);
		if (ret < 0) {
			aw_dev_err(aw883xx->dev," go to on failed");
			return ret;
		}
	} else {
		mutex_lock(&aw883xx->lock);
...
EXPORT_SYMBOL(aw883xx_set_pa_ctl);

static const char *const aw883xx_mute_function[] = { "Off", "On" };

static const struct soc_enum aw883xx_mute_snd_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(aw883xx_mute_function), aw883xx_mute_function),
};

static int aw883xx_mute_get_status(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
static int aw883xx_mute_put_status(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec = aw_componet_codec_ops.kcontrol_codec(kcontrol);
	struct aw883xx *aw883xx = aw_componet_codec_ops.codec_get_drvdata(codec);
	...
	if (ucontrol->value.integer.value[0]) {
		aw883xx_set_pa_ctl(aw883xx, false);
	} else {
		aw883xx_set_pa_ctl(aw883xx, true);
	}

	pr_info("%s(), speaker_mute_control = %ld\n", __func__, ucontrol->value.integer.value[0]);
	return 0;
}
...
static struct snd_kcontrol_new aw883xx_controls[] = {
	...
	SOC_ENUM_EXT("Speaker_Mute_Switch", aw883xx_mute_snd_enum[0],
		aw883xx_mute_get_status, aw883xx_mute_put_status),
};
...
/*2023/05/16, Add for smartpa err feedback.*/
	aw_componet_codec_ops.add_codec_controls(aw883xx->codec,
			&aw883xx_check_feedback[0], ARRAY_SIZE(aw883xx_check_feedback));
#endif
        }
}
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
/*2023/05/16, Add for smartpa err feedback.*/
/*MTK platform set aw_dev_0_switch before pcm close, so check reg here rather than aw883xx_mute */
			if (g_chk_err) {
				aw883xx_check_status_reg(aw883xx);
				g_chk_err = false;
			}
...
#endif
{
	...
	if (gpio_to_irq(aw883xx->irq_gpio)) {
		desc = gpio_to_desc(aw883xx->irq_gpio);
	...
	if (gpio_is_valid(aw883xx->reset_gpio)) {
		desc = gpio_to_desc(aw883xx->reset_gpio);
		devm_gpiod_put(&i2c->dev, desc);
	}
#else
	if (gpio_to_irq(aw883xx->irq_gpio))
