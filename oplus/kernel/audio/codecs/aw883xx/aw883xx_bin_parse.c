/*
 * aw883xx_bin_parse.c aw883xx codec driver
 *
...
static char *profile_name[AW_PROFILE_MAX] = {"Music", "Voice", "Voip",
					"Ringtone", "Ringtone_hs", "Lowpower",
...
static int aw_dev_parse_data_by_sec_type(struct aw_device *aw_dev,
		struct aw_cfg_hdr *cfg_hdr, struct aw_cfg_dde *cfg_dde,
		struct aw_prof_desc *scene_prof_desc)
{
	...
	case ACF_SEC_TYPE_MUTLBIN:
		return aw_dev_prof_parse_multi_bin(aw_dev,
				...
				cfg_dde->data_size, scene_prof_desc);
	default:
		aw_dev_err(aw_dev->dev, "unsupported type:%u", cfg_dde->data_type);
		return -EINVAL;
	}
	return 0;
}
