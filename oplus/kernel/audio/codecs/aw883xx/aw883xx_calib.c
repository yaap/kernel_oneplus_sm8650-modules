...
#ifdef AW883XX_NEED_CONTROL_CALIB
...
static int aw_cali_svc_get_dev_f0_range(struct aw_device *aw_dev,
						uint32_t *data_buf)
{
	data_buf[F0_MIN_FLAG] = aw_dev->f0_range.f0_min;
	data_buf[F0_MAX_FLAG] = aw_dev->f0_range.f0_max;
...
static ssize_t aw_f0_range_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	struct aw883xx *aw883xx = dev_get_drvdata(dev);
	struct aw_device *aw_dev = aw883xx->aw_pa;
	uint32_t range_buf[F0_RANGE_NUM] = { 0 };

	aw_cali_svc_get_dev_f0_range(aw_dev, range_buf);

	len += snprintf(buf + len, PAGE_SIZE - len,
		"f0_min value: [%d]\n", range_buf[F0_MIN_FLAG]);
	len += snprintf(buf + len, PAGE_SIZE - len,
		"f0_max value: [%d]\n", range_buf[F0_MAX_FLAG]);
