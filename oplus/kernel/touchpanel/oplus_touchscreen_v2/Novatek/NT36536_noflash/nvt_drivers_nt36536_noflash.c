...
#else
static void nvt_read_u8_data(struct chip_data_nt36536 *chip_info,
			   uint32_t xdata_addr, int8_t *xdata, int32_t xdata_len)
{
	int32_t i = 0;
	int32_t j = 0;
	int32_t k = 0;
	...
	if (xdata_len / sizeof(uint8_t) < data_len) {
	...
	/*malloc buffer space*/
	xdata_tmp_len = dummy_len + data_len + residual_len;
	xdata_tmp = tp_devm_kzalloc(&chip_info->s_client->dev, xdata_tmp_len, GFP_KERNEL);
	...
	/*---remove dummy data---*/
	for (i = 0; i < data_len; i++) {
		xdata[i] = (int16_t)(xdata_tmp[dummy_len + i]);
	}

	/*---set xdata index to EVENT BUF ADDR---*/
	nvt_set_page(chip_info, chip_info->trim_id_table.mmap->EVENT_BUF_ADDR);

	tp_devm_kfree(&chip_info->s_client->dev, (void **)&xdata_tmp, xdata_tmp_len);
}
#endif
