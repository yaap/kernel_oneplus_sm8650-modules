...
#include "nvt_drivers_nt36532c.h"
...
static void nvt_sw_reset_idle_no_delay(struct chip_data_nt36523 *chip_info)
{
	/*---MCU idle cmds to SWRST_SIF_ADDR--- */
	TPD_INFO("%s is called!\n", __func__);
	nvt_write_addr(chip_info->s_client, chip_info->swrst_sif_addr, 0xAA);
...
void nvt_clear_fw_reset_state(struct chip_data_nt36523 *chip_info)
{
	TPD_INFO("%s is called!\n", __func__);
	nvt_write_addr(chip_info->s_client,
			chip_info->trim_id_table.mmap->EVENT_BUF_ADDR | EVENT_MAP_RESET_COMPLETE,
			0x00);
}
...
return:
	n.a.
*******************************************************/
static void nvt_bootloader_reset(struct chip_data_nt36523 *chip_info)
{
	/*---reset cmds to SWRST_SIF_ADDR--- */
	TPD_DEBUG("%s is called!\n", __func__);
	nvt_write_addr(chip_info->s_client, chip_info->swrst_sif_addr, 0x69);

	mdelay(35);  /* need 35ms delay after bootloader reset */
}
...
static void nvt_stop_crc_reboot(struct chip_data_nt36523 *chip_info)
{
	...
	nvt_set_page(chip_info, chip_info->chip_ver_trim_addr);

	/* ---read to check if buf is 0xFC which means IC is in CRC reboot --- */
	buf[0] = chip_info->chip_ver_trim_addr & 0xFF;
	CTP_SPI_READ(chip_info->s_client, buf, 4);
	...
	if ((buf[1] == 0xFC) ||
		...
		for (retry = 5; retry > 0; retry--) {
			/*---reset idle : 1st--- */
			nvt_sw_reset_idle_no_delay(chip_info);
		...
		struct nvt_ts_hw_reg_addr_info hw_regs)
{
	...
	nvt_bootloader_reset(chip_info); /* NOT in retry loop */
...
*******************************************************/
static int32_t nvt_check_fw_reset_state(
...
static int32_t nvt_get_fw_info(struct chip_data_nt36523 *chip_info)
{
...
static void nova_debug_info(struct chip_data_nt36523 *chip_info, u8 *buf)
{
	...
	chip_info->nvt_fw_debug_info.diff_abnormal = (uint8_t)(buf[0] >> 7) & 0x01;
	...
	chip_info->nvt_fw_debug_info.hopping_1s = (uint8_t)(buf[2] >> 3) & 0x01;
	...
	/*msleep(2000);*/
	if (tp_debug > 0) {
		pr_err("DIFF_ABNORMAL:0x%02X, REK_INFO:0x%02X, RST_INFO:0x%02X\n",
				chip_info->nvt_fw_debug_info.diff_abnormal,
		...
		pr_err("HOPPING:0x%02X, HOPPING_1S:0x%02X, FOG:0x%02X, FILM:0x%02X, NOTCH:0x%02X\n\n",
			   chip_info->nvt_fw_debug_info.hopping,
			   chip_info->nvt_fw_debug_info.hopping_1s,
...
#if NVT_REALTIME_DATA
static void nvt_read_realtime_data(struct chip_data_nt36523 *chip_info)
{
	char str[1024];
	int i, j;
	int remaining;
	int ret;
	int realtime_data_len = chip_info->hw_res->tx_num * chip_info->hw_res->rx_num * 2;

	if (realtime_data_len > NVT_REALTIME_DATA_SIZE) {
		TPD_INFO("realtime data size error\n");
	}

	memset(chip_info->realtime_data, 0, NVT_REALTIME_DATA_SIZE);

	nvt_read_mdata(chip_info, NVT_REALTIME_DATA_ADDR, chip_info->realtime_data, realtime_data_len);

	/* print data */
	for (i = 0; i < chip_info->hw_res->rx_num; i++) {
		ret = 0;
		for (j = 0; j < chip_info->hw_res->tx_num; j++) {
			remaining = sizeof(str) - ret;
			if (remaining > 0) {
				ret += snprintf(str + ret, remaining, "%5d, ",
						chip_info->realtime_data[i * chip_info->hw_res->tx_num + j]);
			} else {
				TPD_INFO("str buffer size error\n");
			}
		}
		str[sizeof(str) - 1] = '\0';
		TPD_INFO("%s\n", str);
...
#if NVT_REALTIME_DATA
	nvt_read_realtime_data(chip_info);
#endif
							...
							uint32_t xdata_addr, u8 *xdata, int32_t xdata_len)
{
	...
	int32_t j = 0;
	int32_t k = 0;
	uint8_t buf[SPI_TRANSFER_LENGTH + 2] = {0};
	...
	for (i = 0; i < ((dummy_len + xdata_len) / XDATA_SECTOR_SIZE); i++) {
		/*---read xdata by SPI_TRANSFER_LENGTH*/

		for (j = 0; j < (XDATA_SECTOR_SIZE / SPI_TRANSFER_LENGTH); j++) {
			/*---change xdata index---*/
			nvt_set_page(chip_info, head_addr + (XDATA_SECTOR_SIZE * i) +
					 (SPI_TRANSFER_LENGTH * j));

			/*---read data---*/
			buf[0] = SPI_TRANSFER_LENGTH * j;
			CTP_SPI_READ(chip_info->s_client, buf, SPI_TRANSFER_LENGTH + 1);

			/*---copy buf to xdata_tmp---*/
			for (k = 0; k < SPI_TRANSFER_LENGTH; k++) {
				xdata_tmp[XDATA_SECTOR_SIZE * i + SPI_TRANSFER_LENGTH * j + k] = buf[k + 1];
				/*printk("0x%02X, 0x%04X\n", buf[k+1], (XDATA_SECTOR_SIZE*i + SPI_TRANSFER_LENGTH*j + k));*/
			}
		}
	...
	if (residual_len != 0) {
		...
		for (j = 0; j < (residual_len / SPI_TRANSFER_LENGTH + 1); j++) {
			/*---change xdata index---*/
			...
			/*---copy buf to xdata_tmp---*/
			for (k = 0; k < SPI_TRANSFER_LENGTH; k++) {
				xdata_tmp[(dummy_len + xdata_len - residual_len) + SPI_TRANSFER_LENGTH * j + k] =
					buf[k + 1];
				/*printk("0x%02X, 0x%04x\n", buf[k+1], ((dummy_len+data_len-residual_len) + SPI_TRANSFER_LENGTH*j + k));*/
			}
		}
...
static int nvt_reset(void *chip_data)
{
	...
	gpio_direction_input(ts->hw_res.irq_gpio);
...
static __maybe_unused int nvt_set_pen_jitter_para(struct chip_data_nt36523 *chip_info, int level)
{
	int8_t ret = -1;
			   ...
			   uint32_t xdata_addr, int32_t *xdata, int32_t xdata_len)
{
	...
	if (residual_len != 0) {
		...
		for (j = 0; j < (residual_len / SPI_TRANSFER_LENGTH + 1); j++) {
			...
			/*---copy buf to xdata_tmp---*/
			for (k = 0; k < SPI_TRANSFER_LENGTH; k++) {
				xdata_tmp[(dummy_len + data_len - residual_len) + SPI_TRANSFER_LENGTH * j + k] =
					buf[k + 1];
				/*printk("0x%02X, 0x%04x\n", buf[k+1], ((dummy_len+data_len-residual_len) + SPI_TRANSFER_LENGTH*j + k));*/
			}
		}
		...
		uint32_t xdata_addr, int32_t *xdata, uint32_t num)
{
	...
	for (i = 0; i < ((dummy_len + data_len) / XDATA_SECTOR_SIZE); i++) {
		...
		for (j = 0; j < (XDATA_SECTOR_SIZE / SPI_TRANSFER_LENGTH); j++) {
		/* ---change xdata index--- */
		nvt_set_page(chip_info, head_addr + (XDATA_SECTOR_SIZE * i) + (SPI_TRANSFER_LENGTH * j));
	...
	if (residual_len != 0) {
		...
		for (j = 0; j < (residual_len / SPI_TRANSFER_LENGTH + 1); j++) {
		/* ---change xdata index--- */
		nvt_set_page(chip_info, xdata_addr + data_len - residual_len + (SPI_TRANSFER_LENGTH * j));
		...
		/* ---copy buf to xdata_tmp--- */
		for (k = 0; k < SPI_TRANSFER_LENGTH; k++) {
				xdata_tmp[(dummy_len + data_len - residual_len) + SPI_TRANSFER_LENGTH * j + k] = buf[k + 1];
				/* printk("0x%02X, 0x%04x\n", buf[k+1], ((dummy_len+data_len-residual_len) + SPI_TRANSFER_LENGTH*j + k)); */
		}
		}
		/* printk("addr=0x%05X\n", (xdata_addr+data_len-residual_len)); */
				 ...
				 int32_t xdata_len)
{
	...
	/* need wait PS_Config_Diff_Test_Frame * 33.2ms*/
	msleep(frame_num * 332);
...
int32_t Switch_GCM(struct chip_data_nt36523 *chip_info, uint8_t enable)
{
	int32_t ret = 0;
	uint8_t buf[8] = {0};
	int32_t retry = 0;
	uint32_t GCM_CODE_ADDR = chip_info->trim_id_table.mmap->GCM_CODE_ADDR;
	uint32_t GCM_FLAG_ADDR = chip_info->trim_id_table.mmap->GCM_FLAG_ADDR;

	while (1) {
		nvt_set_page(chip_info, GCM_CODE_ADDR);
		if (ret < 0) {
			TPD_INFO("change buffer index error!!(%d)\n", ret);
		...
		CTP_SPI_WRITE(chip_info->s_client, buf, 4);
		if (ret < 0) {
			TPD_INFO("Write Switch GCM error!!(%d)\n", ret);
			goto Switch_GCM_exit;
		}
		/* check gcm flag */
		...
		CTP_SPI_READ(chip_info->s_client, buf, 2);
		if (ret < 0) {
			TPD_INFO("Read GCM flag error!!(%d)\n", ret);
			goto Switch_GCM_exit;
		}
		if (enable) {
			if ((buf[1] & 0x01) == 0x01) {
		...
		retry++;
		if (unlikely(retry > 3)) {
			TPD_INFO("Switch GCM %d failed!!(0x%02X)\n", enable, buf[1]);
			ret = -1;
			goto Switch_GCM_exit;
		}
	}
...
int32_t Init_GCM(struct chip_data_nt36523 *chip_info)
{
	int32_t ret = 0;

	/* Enter GCM mode */
	ret = Switch_GCM(chip_info, 1);
	if (ret) {
		TPD_INFO("Enable GCM mode failed!!(%d)\n", ret);
		ret = -1;
		goto Init_GCM_exit;
	} else {
		TPD_INFO("Init OK\n");
...
int32_t nvt_gcm_xfer(struct chip_data_nt36523 *chip_info, gcm_xfer_t *xfer)
{
	int32_t ret = 0;
	...
	uint32_t FLASH_CMD_ADDR = chip_info->trim_id_table.mmap->FLASH_CMD_ADDR;
	uint32_t FLASH_CMD_ISSUE_ADDR = chip_info->trim_id_table.mmap->FLASH_CMD_ISSUE_ADDR;
	uint32_t RW_FLASH_DATA_ADDR = chip_info->trim_id_table.mmap->RW_FLASH_DATA_ADDR;
	int32_t write_len = 0;
	uint32_t tmp_addr = 0;
	int32_t tmp_len = 0;
	int32_t i;
	...
	if (SPI_TRANSFER_LENGTH <= FLASH_PAGE_SIZE)
		transfer_len = SPI_TRANSFER_LENGTH;
	else
	...
	if (!buf) {
		TPD_INFO("alloc buf failed! total_buf_size=%d\n", total_buf_size);
		ret = -1;
	...
	if ((xfer->tx_len > 0) && xfer->tx_buf) {
		for (i = 0; i < xfer->tx_len; i += transfer_len) {
			...
			nvt_set_page(chip_info, tmp_addr);
			buf[0] = tmp_addr & 0xFF;
			memcpy(buf + 1, xfer->tx_buf + i, tmp_len);

			CTP_SPI_WRITE(chip_info->s_client, buf, 1 + tmp_len);
			if (ret < 0) {
				TPD_INFO("Write tx data error!!(%d)\n", ret);
				goto nvt_gcm_xfer_exit;
			}
		}
	}

	ret = nvt_set_page(chip_info, FLASH_CMD_ADDR);
	if (ret < 0) {
		TPD_INFO("change buffer index error!!(%d)\n", ret);
		goto nvt_gcm_xfer_exit;
	}
	...
	write_len = xfer->flash_addr_len + xfer->tx_len;
	if (write_len > 0) {
	...
	ret = CTP_SPI_WRITE(chip_info->s_client, buf, 13);
	if (ret < 0) {
		TPD_INFO("Write Enter GCM error!!(%d)\n", ret);
		goto nvt_gcm_xfer_exit;
	}
	...
	while (1) {
		...
		ret = CTP_SPI_READ(chip_info->s_client, buf, 2);
		if (ret < 0) {
			TPD_INFO("Read FLASH_CMD_ISSUE_ADDR status error!!(%d)\n", ret);
			goto nvt_gcm_xfer_exit;
		}
		if (buf[1] == 0x00) {
			/* TPD_INFO("wait_cmd_issue_cnt=%d\n", wait_cmd_issue_cnt); */
			break;
		}
		wait_cmd_issue_cnt++;
		if (unlikely(wait_cmd_issue_cnt > 2000)) {
			TPD_INFO("write GCM cmd 0x%02X failed!!(0x%02X)\n", xfer->flash_cmd, buf[1]);
			ret = -1;
			goto nvt_gcm_xfer_exit;
		}
		mdelay(1);
	...
	if ((xfer->rx_len > 0) && xfer->rx_buf) {
		...
		for (i = 0; i < xfer->rx_len; i += transfer_len) {
			...
			nvt_set_page(chip_info, tmp_addr);
			buf[0] = tmp_addr & 0xFF;
			ret = CTP_SPI_READ(chip_info->s_client, buf, 1 + tmp_len);
			if (ret < 0) {
				TPD_INFO("Read rx data fail error!!(%d)\n", ret);
				goto nvt_gcm_xfer_exit;
			}
...
int32_t Check_FW_Ver(struct chip_data_nt36523 *chip_info,
		const struct firmware *fw)
{
	uint8_t buf[16] = {0};
	int32_t ret = 0;

	/* write index to EVENT BUF ADDR */
	ret = nvt_set_page(chip_info, chip_info->trim_id_table.mmap->EVENT_BUF_ADDR | EVENT_MAP_FWINFO);
	if (ret < 0) {
		TPD_INFO("write index error!(%d)\n", ret);
		return ret;
	}
	...
	if (buf[1] > fw->data[chip_info->fw_need_write_size - SIZE_4KB])
		return 1;
	else
		return 0;
}

int32_t Resume_PD_GCM(struct chip_data_nt36523 *chip_info)
{
	int32_t ret = 0;
	gcm_xfer_t xfer;
	...
	xfer.flash_cmd = 0xAB;
	ret = nvt_gcm_xfer(chip_info, &xfer);
	if (ret) {
		TPD_INFO("Resume PD failed!!(%d)\n", ret);
		ret = -1;
	} else {
...
int32_t Get_Checksum_GCM(struct chip_data_nt36523 *chip_info,
		uint32_t flash_addr, uint16_t data_len, uint16_t *checksum)
{
	int32_t ret = 0;
	gcm_xfer_t xfer;
	uint32_t READ_FLASH_CHECKSUM_ADDR = chip_info->trim_id_table.mmap->READ_FLASH_CHECKSUM_ADDR;
	uint8_t buf[8] = {0};
	...
	/* TPD_INFO("%s: flash_addr=0x%04X, data_len=%d\n", __func__, flash_addr, data_len); */
	ret = nvt_gcm_xfer(chip_info, &xfer);
	if (ret) {
		TPD_INFO("Get Checksum GCM fail!!, ret = %d\n", ret);
		ret = -1;
		goto Get_Checksum_GCM_exit;
	}

	nvt_set_page(chip_info, READ_FLASH_CHECKSUM_ADDR);
	buf[0] = READ_FLASH_CHECKSUM_ADDR & 0xFF;
	buf[1] = 0x00;
	buf[2] = 0x00;
	ret = CTP_SPI_READ(chip_info->s_client, buf, 3);
	if (ret < 0) {
		TPD_INFO("Get checksum error!!(%d)\n", ret);
		ret = -1;
		goto Get_Checksum_GCM_exit;
	}
	*checksum = (buf[2] << 8) | buf[1];

	/* TPD_INFO("Get Checksum OK.\n"); */
	ret = 0;
...
*******************************************************/
int32_t Write_Enable_GCM(struct chip_data_nt36523 *chip_info)
{
	int32_t ret = 0;
	gcm_xfer_t xfer;
	...
	ret = nvt_gcm_xfer(chip_info, &xfer);
	if (ret) {
		TPD_INFO("Write Enable failed!!(%d)\n", ret);
		ret = -1;
	} else {
		/* TPD_INFO("Write Enable OK\n"); */
		ret = 0;
	}
...
return:
	Executive outcomes. 0---succeed. negative---failed.
*******************************************************/
int32_t Write_Status_GCM(struct chip_data_nt36523 *chip_info, uint8_t status)
{
	int32_t ret = 0;
	gcm_xfer_t xfer;
	...
	ret = nvt_gcm_xfer(chip_info, &xfer);
	if (ret) {
		TPD_INFO("Write Status GCM fail!!(%d)\n", ret);
		ret = -1;
	} else {
		/* TPD_INFO("Write Status Register OK.\n"); */
		ret = 0;
	}
...
return:
	Executive outcomes. 0---succeed. negative---failed.
*******************************************************/
int32_t Read_Status_GCM(struct chip_data_nt36523 *chip_info, uint8_t *status)
{
	int32_t ret = 0;
	gcm_xfer_t xfer;
	...
	ret = nvt_gcm_xfer(chip_info, &xfer);
	if (ret) {
		TPD_INFO("Read Status GCM fail!!(%d)\n", ret);
		ret = -1;
	} else {
		/* TPD_INFO("Read Status Register OK. status=0x%02X\n", *status); */
		ret = 0;
	}
...
return:
	Executive outcomes. 0---succeed. negative---failed.
*******************************************************/
int32_t Sector_Erase_GCM(struct chip_data_nt36523 *chip_info, uint32_t flash_addr)
{
	int32_t ret = 0;
	gcm_xfer_t xfer;
	...
	ret = nvt_gcm_xfer(chip_info, &xfer);
	if (ret) {
		TPD_INFO("Sector Erase GCM fail!!(%d)\n", ret);
		ret = -1;
	} else {
		/* TPD_INFO("Sector Erase GCM OK.\n"); */
		ret = 0;
	}
...
return:
	Executive outcomes. 0---succeed. negative---failed.
*******************************************************/
int32_t Erase_Flash_GCM(struct chip_data_nt36523 *chip_info)
{
	int32_t ret = 0;
	int32_t count = 0;
	int32_t i = 0;
	...
	/* Write Enable */
	ret = Write_Enable_GCM(chip_info);
	if (ret < 0) {
		TPD_INFO("Write Enable (for Write Status Register) error!!(%d)\n", ret);
		ret = -1;
		goto Erase_Flash_GCM_exit;
	}

	/* Write Status Register */
	ret = Write_Status_GCM(chip_info, 0x00);
	if (ret < 0) {
		TPD_INFO("Write Status Register error!!(%d)\n", ret);
		ret = -1;
		goto Erase_Flash_GCM_exit;
	}
	...
	while (1) {
		mdelay(5);
		ret = Read_Status_GCM(chip_info, &status);
		if (ret < 0) {
			TPD_INFO("Read Status Register error!!(%d)\n", ret);
			ret = -1;
			goto Erase_Flash_GCM_exit;
		}
		if ((status & 0x03) == 0x00) {
			/* TPD_INFO("%s:%d, retry=%d\n", __func__, __LINE__, retry); */
			break;
		}
		retry++;
		if (unlikely(retry > 20)) {
			TPD_INFO("Read Status failed!!(0x%02X)\n", status);
			ret = -1;
			goto Erase_Flash_GCM_exit;
		}
	}

	if (chip_info->fw_need_write_size % FLASH_SECTOR_SIZE)
		count = chip_info->fw_need_write_size / FLASH_SECTOR_SIZE + 1;
	else
		count = chip_info->fw_need_write_size / FLASH_SECTOR_SIZE;

	for (i = 0; i < count; i++) {
		/* Write Enable */
		ret = Write_Enable_GCM(chip_info);
		if (ret < 0) {
			TPD_INFO("Write Enable error!!(%d,%d)\n", ret, i);
			ret = -1;
			goto Erase_Flash_GCM_exit;
		}
		...
		/* Sector Erase */
		ret = Sector_Erase_GCM(chip_info, Flash_Address);
		if (ret < 0) {
			TPD_INFO("Sector Erase error!!(%d,%d)\n", ret, i);
			ret = -1;
			goto Erase_Flash_GCM_exit;
		}
		mdelay(25);
		...
		while (1) {
			...
			if (unlikely(retry > 100)) {
				TPD_INFO("Wait Sector Erase timeout!!\n");
				ret = -1;
				goto Erase_Flash_GCM_exit;
			}
			mdelay(1);
...
return:
	Executive outcomes. 0---succeed. negative---failed.
*******************************************************/
int32_t Page_Program_GCM(struct chip_data_nt36523 *chip_info,
		uint32_t flash_addr, uint16_t data_len, uint8_t *data)
{
	int32_t ret = 0;
	gcm_xfer_t xfer;
	uint16_t checksum = 0;
	...
	ret = nvt_gcm_xfer(chip_info, &xfer);
	if (ret) {
		TPD_INFO("Page Program GCM fail!!(%d)\n", ret);
		ret = -1;
	} else {
		/* TPD_INFO("Page Program GCM OK.\n"); */
		ret = 0;
	}
...
return:
	Executive outcomes. 0---succeed. negative---failed.
*******************************************************/
int32_t Write_Flash_GCM(struct chip_data_nt36523 *chip_info,
		const struct firmware *fw)
{
	int32_t ret = 0;
	uint8_t buf[64] = {0};
	...
	uint32_t FLASH_CKSUM_STATUS_ADDR = chip_info->trim_id_table.mmap->FLASH_CKSUM_STATUS_ADDR;
	int32_t percent = 0;
	int32_t previous_percent = -1;

	if (chip_info->fw_need_write_size % FLASH_PAGE_SIZE)
		count = chip_info->fw_need_write_size / FLASH_PAGE_SIZE + 1;
	else
		count = chip_info->fw_need_write_size / FLASH_PAGE_SIZE;
...
page_program_start:
		...
		/* Page Program */
		ret = Page_Program_GCM(chip_info,
				Flash_Address,
				min(chip_info->fw_need_write_size - Flash_Address, (size_t)FLASH_PAGE_SIZE),
				(uint8_t *)&(fw->data[Flash_Address]));
		if (ret < 0) {
			TPD_INFO("Page Program error!!(%d), i=%d\n", ret, i);
			ret = -1;
			goto Write_Flash_GCM_exit;
		}

		/* check flash checksum status */
		nvt_set_page(chip_info, FLASH_CKSUM_STATUS_ADDR);
		retry = 0;
		while (1) {
			...
			if (buf[1] == 0xAA) { /* checksum pass */
				/* TPD_INFO("page_program_retry=%d, retry=%d\n", page_program_retry, retry); */
				ret = 0;
				break;
				...
				if (page_program_retry < 1) {
					...
					TPD_INFO("Check Flash Checksum Status error!!\n");
					ret = -1;
					goto Write_Flash_GCM_exit;
				}
			}
			retry++;
			if (unlikely(retry > 20)) {
				TPD_INFO("Check flash shecksum fail!!, buf[1]=0x%02X\n", buf[1]);
				ret = -1;
				goto Write_Flash_GCM_exit;
			}
			mdelay(1);
		...
		while (1) {
			...
			if (unlikely(retry > 200)) {
				TPD_INFO("Wait Page Program timeout!!\n");
				ret = -1;
				goto Write_Flash_GCM_exit;
			}
			mdelay(1);
...
return:
	Executive outcomes. 0---succeed. negative---failed.
*******************************************************/
int32_t Verify_Flash_GCM(struct chip_data_nt36523 *chip_info, const struct firmware *fw)
{
	int32_t ret = 0;
	...
	total_sector_need_check = chip_info->fw_need_write_size / SIZE_4KB;
	/* TPD_INFO("total_sector_need_check=%d\n", total_sector_need_check); */
	for (i = 0; i < total_sector_need_check; i++) {
		...
		for (j = 0; j < data_len; j++) {
			write_checksum += fw->data[flash_addr + j];
		}
		write_checksum = ~write_checksum + 1;

		ret = Get_Checksum_GCM(chip_info, flash_addr, data_len, &read_checksum);
		if (ret < 0) {
			TPD_INFO("Get Checksum failed!!(%d,%d)\n", ret, i);
			ret = -1;
			goto Verify_Flash_GCM_exit;
		}
		/* TPD_INFO("i=%d, write_checksum=0x%04X, read_checksum=0x%04X\n", i, write_checksum, read_checksum); .*/
		if (write_checksum != read_checksum) {
			TPD_INFO("Verify Failed!!, i=%d, write_checksum=0x%04X, read_checksum=0x%04X\n",
					i, write_checksum, read_checksum);
			ret = -1;
			goto Verify_Flash_GCM_exit;
		}
...
*******************************************************/
int32_t Check_CheckSum_GCM(struct chip_data_nt36523 *chip_info, const struct firmware *fw)
{
	int32_t ret = 0;
	int32_t i = 0;
	int32_t total_sector_need_check = 0;
	...
	ret = Init_GCM(chip_info);
	if (ret < 0) {
		TPD_INFO("Init GCM error!!(%d)\n", ret);
		goto Check_CheckSum_exit;
	}

	ret = Resume_PD_GCM(chip_info);
	if (ret < 0) {
		TPD_INFO("Resume PD error!!(%d)\n", ret);
		goto Check_CheckSum_exit;
	}
	...
	for (i = 0; i < total_sector_need_check; i++) {
		...
		/* TPD_INFO("i=%d, write_checksum=0x%04X, read_checksum=0x%04X\n", i, write_checksum, read_checksum); */
		if (write_checksum != read_checksum) {
			TPD_INFO("firmware checksum not match!!, i=%d, write_checksum=0x%04X, read_checksum=0x%04X\n",
					i, write_checksum, read_checksum);
			ret = 0;
			goto Check_CheckSum_exit;
		} else {
...
int32_t nvt_check_flash_end_flag_gcm(struct chip_data_nt36523 *chip_info)
{
	...
	memset(&xfer, 0, sizeof(gcm_xfer_t));
	xfer.flash_cmd = 0x03;
	xfer.flash_addr = chip_info->fw_need_write_size - NVT_FLASH_END_FLAG_LEN;
	xfer.flash_addr_len = GCM_FLASH_ADDR_LEN;
	...
	ret = nvt_gcm_xfer(chip_info, &xfer);
	if (ret) {
		TPD_INFO("Get \"NVT\" end flag fail!!, ret = %d\n", ret);
		ret = -1;
		return ret;
	} else {
		/* TPD_INFO("Get \"NVT\" end flag OK.\n"); */
		TPD_INFO("nvt_end_flag=%s (%02X %02X %02X)\n",
	...
	if (strncmp(nvt_end_flag, "NVT", NVT_FLASH_END_FLAG_LEN) == 0) {
		...
		TPD_INFO("\"NVT\" end flag not found!\n");
		return 1;
	}
}

/* TODO: modify to flash program */
...
try_fw_in_dts:
	...
	if (!force) {
		...
		ret = Check_CheckSum_GCM(chip_info, fw);	/* -1:error, 0:not match, 1:match */

		if (ret < 0) {
			/* read firmware checksum failed */
			TPD_INFO("read firmware checksum failed\n");
			goto update_firmware;
		} else if ((ret == 0) && (Check_FW_Ver(chip_info, fw) == 0)) {
			/* (fw checksum not match) && (bin fw version >= ic fw version) */
			TPD_INFO("firmware version not match\n");
			goto update_firmware;
		} else if (nvt_check_flash_end_flag_gcm(chip_info)) {
			TPD_INFO("check flash end flag failed\n");
			goto update_firmware;
		} else {
			/* Bootloader Reset */
			nvt_clear_fw_reset_state(chip_info);
			nvt_bootloader_reset(chip_info);
...
update_firmware:
	...
	ret = Init_GCM(chip_info);
	if (ret) {
		TPD_INFO("Init GCM failed, exit!\n");
		update_state = FW_UPDATE_ERROR;
		goto OUT;
	}

	/* Step 2 : Resume PD */
	TPD_INFO("Resume PD GCM start\n");
	ret = Resume_PD_GCM(chip_info);
	if (ret) {
		TPD_INFO("Resume PD GCM failed, exit!\n");
		update_state = FW_UPDATE_ERROR;
		goto OUT;
	}

	/* Step 3 : Erase */
	TPD_INFO("Erase Flash GCM start\n");
	ret = Erase_Flash_GCM(chip_info);
	if (ret) {
		TPD_INFO("Erase Flash GCM failed, exit!\n");
		update_state = FW_UPDATE_ERROR;
		goto OUT;
	}

	/* Step 4 : Program */
	TPD_INFO("Write Flash GCM start\n");
	ret = Write_Flash_GCM(chip_info, fw);
	if (ret) {
		TPD_INFO("Write Flash GCM failed, exit!\n");
		update_state = FW_UPDATE_ERROR;
		goto OUT;
	}

	/* Step 5 : Verify */
	TPD_INFO("Verify Flash GCM start\n");
	ret = Verify_Flash_GCM(chip_info, fw);
	if (ret) {
		TPD_INFO("Verify Flash GCM failed, exit!\n");
		update_state = FW_UPDATE_ERROR;
		goto OUT;
	}
...
OUT:
	...
	ret = nvt_get_fw_info(chip_info);
	if (ret) {
		TPD_INFO("nvt_get_fw_info failed. (%d)\n", ret);
		goto out_fail;
	}
...
/*static void nvt_pen_data_read(struct seq_file *s, struct chip_data_nt36523 *chip_info, DEBUG_READ_TYPE read_type)
{
	...
	ret = nvt_check_fw_reset_state(chip_info, RESET_STATE_NORMAL_RUN);
	if (ret < 0) {
					 ...
					 struct auto_testdata *nvt_testdata, struct test_item_info *p_test_item_info)
{
	...
	if (nvt_check_fw_reset_state(chip_info, RESET_STATE_REK)) {
		TPD_INFO("check fw reset state failed!\n");
...
static int nvt_tp_probe(struct spi_device *client)
{
...
#if NVT_REALTIME_DATA
	chip_info->realtime_data = kzalloc(NVT_REALTIME_DATA_SIZE, GFP_KERNEL);
	if (chip_info->realtime_data == NULL) {
		TPD_INFO("realtime_data malloc memory failed!\n");
		goto err_realtime_data;
	}
#endif
	...
	nvt_flash_proc_init(ts, "nvt_tddi_info");
...
MODULE_DESCRIPTION("Novatek NT36532/NT36536 with Flash Driver");
MODULE_LICENSE("GPL");
